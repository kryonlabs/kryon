...
overlay_frame(Vector2 mouse, int released)
{
    ...
    scrim.a = 92;
    mouse_released = 0;
    BeginInterfaceFrame(320, 560, 1.0f);
    SetMouseWorldOverride(1, mouse);
...
finish_frame(void)
{
    SetMouseWorldOverride(0, (Vector2){0});
    mouse_released = 0;
    EndInterfaceFrame();
}
...
test_outside_release_closes_and_consumes(void)
{
    ...
    check_int("ui release consumed", ReleaseConsumed(), 1);
...
test_inside_release_keeps_overlay_open(void)
{
    ...
    check_int("inside point not captured",
              InputCapturesClick((Vector2){120.0f, 120.0f}), 0);
    check_int("outside point captured",
              InputCapturesClick((Vector2){24.0f, 120.0f}), 1);
    finish_frame();
}
