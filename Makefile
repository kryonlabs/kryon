...
# Limbo language plugin (enabled for TaijiOS builds)
ifneq ($(filter limbo,$(PLUGINS)),)
    LANGUAGE_SRC += $(SRC_DIR)/runtime/languages/limbo_language.c
    RUNTIME_SRC += $(SRC_DIR)/runtime/limbo/limbo_runtime.c
    RUNTIME_SRC += $(SRC_DIR)/runtime/limbo_builtin.c

    # Add TaijiOS include paths and libraries
    TAIJIOS_DIR ?= /home/wao/Projects/TaijiOS
    INCLUDES += -I$(TAIJIOS_DIR)/include -I$(SRC_DIR)/runtime/limbo
    LDFLAGS += -L$(TAIJIOS_DIR)/Linux/amd64/lib -linterp -l9 -lbio

    CFLAGS += -DKRYON_PLUGIN_LIMBO
    $(info Building with Limbo language plugin for TaijiOS)
endif
...
# KRBView renderer - Check for TaijiOS and renderer implementation
TAIJIOS_DIR ?= /home/wao/Projects/TaijiOS
ifneq ($(wildcard $(TAIJIOS_DIR)/include/krbview.h),)
    ifneq ($(wildcard $(SRC_DIR)/renderers/krbview/krbview_renderer.c),)
        RENDERERS_SRC += $(SRC_DIR)/renderers/krbview/krbview_renderer.c
        CFLAGS += -DHAVE_RENDERER_KRBVIEW=1
        INCLUDES += -I$(TAIJIOS_DIR)/include
