...
func TestPaintLayersNestedOrderAndPixels(t *testing.T) {
	...
	r.BeginFrame()
	r.Box(NewRectangle(0, 0, 80, 80), BLACK, BLANK)
	parent := r.beginPaintLayer(1)
	r.Box(NewRectangle(10, 10, 60, 60), GREEN, BLANK)
	child := r.beginPaintLayer(2)
	r.Box(NewRectangle(20, 20, 30, 30), BLUE, BLANK)
	r.endPaintLayer(child)
	// Even parent paint declared after the nested popup stays below it.
	r.Box(NewRectangle(25, 25, 10, 10), YELLOW, BLANK)
	r.endPaintLayer(parent)
	r.Box(NewRectangle(0, 0, 80, 80), RED, BLANK)
	sibling := r.beginPaintLayer(3)
	r.Box(NewRectangle(40, 40, 20, 20), ORANGE, BLANK)
...
func TestPaintLayersOwnerRemovalAndFrameReset(t *testing.T) {
	...
	r.BeginFrame()
	parent := r.beginPaintLayer(1)
	r.Box(NewRectangle(0, 0, 10, 10), RED, BLANK)
	child := r.beginPaintLayer(2)
	r.Box(NewRectangle(0, 0, 10, 10), GREEN, BLANK)
	r.endPaintLayer(child)
	r.endPaintLayer(parent)
