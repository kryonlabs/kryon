...
static bool apply_property(ConversionContext* ctx, IRComponent* component, const char* name, KryValue* value) {
    ...
    // Try new dispatch table first - handles most common properties efficiently
    ...
    // Property not in dispatch table - fall through to legacy code below
    fprintf(stderr, "[APPLY_PROPERTY] Not in dispatch table, using legacy code: %s\n", name);
...
char* ir_kry_to_kir(const char* source, size_t length) {
    ...
    ctx.base_directory = NULL;                  // Will be set from source file path if available
