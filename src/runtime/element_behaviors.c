...
bool element_behavior_system_init(void) {
    ...
    fprintf(stderr, "✓ Element Behavior System initialized\n");
    return true;
}
...
bool element_behavior_register(const ElementBehavior* behavior) {
    if (!behavior || !behavior->name) {
        fprintf(stderr, "ERROR: Invalid behavior (NULL or no name)\n");
        return false;
    }
    
    if (g_behavior_system.behavior_count >= MAX_BEHAVIORS) {
        fprintf(stderr, "ERROR: Maximum number of behaviors (%d) exceeded\n", MAX_BEHAVIORS);
        return false;
    }
    ...
    for (size_t i = 0; i < g_behavior_system.behavior_count; i++) {
        if (strcmp(g_behavior_system.behaviors[i]->name, behavior->name) == 0) {
            fprintf(stderr, "ERROR: Behavior '%s' already registered\n", behavior->name);
            return false;
        }
    }
    
    g_behavior_system.behaviors[g_behavior_system.behavior_count++] = behavior;
    fprintf(stderr, "✓ Registered behavior: %s\n", behavior->name);
    return true;
}
...
ElementBehaviorState* element_get_behavior_state(struct KryonElement* element) {
    ...
    if (!element->user_data) {
        ...
        if (!state) {
            fprintf(stderr, "ERROR: Failed to allocate element behavior state\n");
            return NULL;
        }
...
bool element_definition_register(const ElementDefinition* definition) {
    if (!definition || !definition->type_name || !definition->behavior_names) {
        fprintf(stderr, "ERROR: Invalid element definition\n");
        return false;
    }
    
    if (g_behavior_system.definition_count >= MAX_ELEMENT_DEFINITIONS) {
        fprintf(stderr, "ERROR: Maximum element definitions (%d) exceeded\n", MAX_ELEMENT_DEFINITIONS);
        return false;
    }
    
    if (g_composed_vtable_count >= MAX_ELEMENT_DEFINITIONS) {
        fprintf(stderr, "ERROR: Maximum composed VTables (%d) exceeded\n", MAX_ELEMENT_DEFINITIONS);
        return false;
    }
    ...
    for (const char** name = definition->behavior_names; *name; name++) {
        if (!element_behavior_get(*name)) {
            fprintf(stderr, "ERROR: Behavior '%s' not found for element '%s'\n", *name, definition->type_name);
            return false;
        }
        behavior_count++;
    }
    ...
    if (behavior_count == 0) {
        fprintf(stderr, "ERROR: Element '%s' has no behaviors\n", definition->type_name);
        return false;
    }
    ...
    if (!composed->behaviors) {
        fprintf(stderr, "ERROR: Failed to allocate behavior array for element '%s'\n", definition->type_name);
    ...
    if (!element_register_type(definition->type_name, &composed->base_vtable)) {
        fprintf(stderr, "ERROR: Failed to register element type '%s'\n", definition->type_name);
    ...
    fprintf(stderr, "✓ Registered element: %s (", definition->type_name);
    for (size_t i = 0; i < behavior_count; i++) {
        fprintf(stderr, "%s", composed->behaviors[i]->name);
        if (i < behavior_count - 1) fprintf(stderr, ", ");
    }
    fprintf(stderr, ")\n");
...
bool element_initialize_behaviors(struct KryonElement* element) {
    ...
    if (composed->definition->custom_init) {
        if (!composed->definition->custom_init(element)) {
            fprintf(stderr, "ERROR: Custom init failed for element '%s'\n", element->type_name);
            return false;
        }
    }
    ...
    for (size_t i = 0; i < composed->behavior_count; i++) {
        ...
        if (behavior && behavior->init) {
            if (!behavior->init(element)) {
                fprintf(stderr, "ERROR: Behavior '%s' init failed for element '%s'\n", 
