                 ...
                 int font, const char *typeface, size_t typeface_length)
{
    ...
    if(length == 3 && memcmp(value, "Off", length) == 0) return 21;
    assert(length == 2 && memcmp(value, "On", length) == 0);
    return 14;
}
...
{
    (void)context; (void)h; (void)g; (void)b; (void)a;
    assert(radius == 0.5f && segments == 16);
    if(x == 10 && y == 20 && w == 54) {
        assert(r == (fills < 4 ? 0xd1 : 0x12));
    }
    fills++;
}
                 ...
                 uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
    ...
    assert(font == 14 && y == 69 &&
           clip[0] == 10 && clip[1] == 60 &&
           clip[2] == 68 && clip[3] == 32);
    assert((length == 3 && memcmp(value, "Off", length) == 0 && x == 16) ||
           (length == 2 && memcmp(value, "On", length) == 0 && x == 54));
    if((labels / 2 < 2 && length == 2) ||
       (labels / 2 >= 2 && length == 3)) {
...
int main(int argc, char **argv)
{
    ...
    assert(bundle != NULL);
    FontMeasurer fonts = {width, unexpected_height, NULL};
