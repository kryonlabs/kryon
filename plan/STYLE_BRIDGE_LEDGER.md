...
B-004 was removed when Page, Section, Heading, ParagraphText, Link, and Flow
composition moved from `src/ui/ui_page.c` to `src/ui/page.kry`. Their structural
spacing and font fallback are now authored in `.kry`.
...
active-theme storage remain host services; `src/ui/theme_color.kry` now owns
native color resolution, contrast, derived roles, and built-in theme assembly.
`theme-policy-test` and Go tests exercise shared theme fixtures, including
alpha and channel-clamping boundaries. The new native theme surface checks are
authored in `tests/widget_surface_test.kry` and await the user's run approval.
The 13 light and 13 dark catalog palettes now live in
`themes/catalog_light.kss` and `themes/catalog_dark.kss`.
`src/ui/theme_catalog.kry` parses and caches their colors;
`src/ui/theme_identity.kry` owns names, scopes, and localized labels. The C
catalog source has been deleted. The KSS catalog check in
`tests/theme_catalog_test.kry` builds but awaits run approval. B-001 remains
open until live callers use KSS theme selection directly.
