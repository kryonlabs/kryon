...
func TestDropdownOverlayLayerAndCapture(t *testing.T) {
	...
	want := RenderFrame(200, 120, popup).RGBAAt(14, 80)
	if got != want || got.R == RED.R && got.G == RED.G && got.B == RED.B {
...
func TestNativeSelectionAndImageWidgets(t *testing.T) {
	...
	image := ImageProps{AssetPath: "tile.png", Bounds: NewRectangle(10, 90, 48, 32), Fit: ImageFitContain}
	r.BeginFrame()
	r.Image(image)
	ops := r.FrameOps()
	...
	r.QueueTap(80, 100)
	r.BeginFrame()
	if !r.Button(ButtonProps{Bounds: image.Bounds, ImageAssetPath: image.AssetPath, ImageBounds: image.Bounds, ImageSource: image.Source, ImageOrigin: image.Origin, ImageRotation: image.Rotation, ImageFit: int32(image.Fit), ID: 85}) {
...
func TestFocusableChoiceAndImageWidgets(t *testing.T) {
	...
	image := ImageProps{AssetPath: "tile.png", Bounds: NewRectangle(10, 170, 48, 32), Fit: ImageFitContain}
	...
	draw := func(disableFlags bool) {
		...
		if r.Button(ButtonProps{Bounds: image.Bounds, ImageAssetPath: image.AssetPath, ImageBounds: image.Bounds, ImageSource: image.Source, ImageOrigin: image.Origin, ImageRotation: image.Rotation, ImageFit: int32(image.Fit), ID: 904}) {
			activations[904]++
		}
