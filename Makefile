...
FLINT_STATIC_PACKAGE_CFLAGS ?= -I$${includedir} -DHAS_LIBOQS=1 -DHAS_LIBCURL=1 -DCURL_STATICLIB -DKRYON_HAS_CMARK_GFM=1
...
$(STATIC_DIST_ARCHIVE): $(LIB) $(RAYLIB_A) $(FLINT_LIBOQS_A) $(FLINT_CURL_A) $(FLINT_MARKDOWN_DEPS) README.md LICENSE THIRD_PARTY_NOTICES.md CHANGELOG.md docs/API.md examples/package/minimal.c examples/package/markdown.c scripts/check-static-package.sh
	...
	printf '%s\n' \
		...
		'set_target_properties(Flint::flint PROPERTIES IMPORTED_LOCATION "$${FLINT_PACKAGE_PREFIX}/lib/libflint.a" INTERFACE_INCLUDE_DIRECTORIES "$${FLINT_PACKAGE_PREFIX}/include" INTERFACE_COMPILE_DEFINITIONS "HAS_LIBOQS=1;HAS_LIBCURL=1;CURL_STATICLIB;KRYON_HAS_CMARK_GFM=1")' \
