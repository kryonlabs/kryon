...
main(void)
{
    ...
    if(!IsWindowReady()) {
        NativeWindow *win = OpenNativeWindow("kryon-test", 0, 0, 32, 32, 0, BLACK, 1.0f);

        check_int("open without a main window", win == NULL, 1);
    }

    /* Every entry point must stay safe with no window at all. */
    BeginNativeWindow(NULL);
    EndNativeWindow();
    CloseNativeWindow(NULL);
    check_int("clicked on null window", IsNativeWindowClicked(NULL), 0);
    check_int("right clicked on null window", IsNativeWindowRightClicked(NULL), 0);
    check_int("dragged null window", IsNativeWindowDragged(NULL), 0);

    GetNativeWindowPosition(NULL, &x, &y);
    check_int("null window position x", x, 0);
    check_int("null window position y", y, 0);
    GetNativeWindowClickPosition(NULL, &x, &y);
