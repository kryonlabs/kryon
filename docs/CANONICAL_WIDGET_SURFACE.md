...
| `Paragraph` | `UI/Display` | Rich text | `runtime/paragraph.kry`, `runtime/drawing_props.kry` | Partly `.kry-backed` | Metrics/default line-gap, layout spacing, line-step, height, line-stride, alignment policy, and `ParagraphSpec` data are `.kry`; text parsing, line-break array ownership, icon shaping, and drawing remain host support. |
