...
Text.face { typeface: test-semibold; font-size: 26; }
Text.face-control { font-size: 26; }`, "Text Typeface", "") || !SetActiveStylePack("test.text.typeface") {
		t.Fatal("could not register text typeface style")
	}
	r := New(AppConfig{Width: 300, Height: 100}).(*runtime)
	r.BeginFrame()
	defer r.EndFrame()
	r.Text(TextProps{Text: "Actions", ClassName: StyleClassID("face"), Wrap: TextWrapNone})
	r.Text(TextProps{Text: "Actions", ClassName: StyleClassID("face-control"), Wrap: TextWrapNone})
	ops := r.FrameOps()
...
Text.tracked3 { font-size: 18; letter-spacing: 3; }
Text.tracked4 { font-size: 18; letter-spacing: 4; }`, "Text Letter Spacing", "") {
		t.Fatal("text letter-spacing style pack did not register")
	}
	r := New(AppConfig{Width: 300, Height: 100}).(*runtime)
	r.BeginFrame()
	natural := runtimeTextWidth("AéB", 18)
	r.Text(TextProps{Text: "AéB", ClassName: StyleClassID("tracked3"), Wrap: TextWrapNone})
	ops := r.FrameOps()
	op := ops[len(ops)-1]
	...
	r.Text(TextProps{Bounds: Rectangle{X: 10, Y: 30, Width: 200, Height: 30},
		Text: "AB", ClassName: StyleClassID("tracked4"), Align: TextAlignCenter, Wrap: TextWrapNone})
	ops = r.FrameOps()
	op = ops[len(ops)-1]
	...
	r.Text(TextProps{Bounds: Rectangle{X: 10, Y: 60, Width: width, Height: 50},
		Text: "AA BB", ClassName: StyleClassID("tracked3"), Wrap: TextWrapAuto})
	ops = r.FrameOps()
...
func TestTextClipsPartiallyVisibleLineInsteadOfDroppingIt(t *testing.T) {
	...
	r.BeginFrame()
	r.Text(TextProps{Bounds: Rectangle{X: 2, Y: 2, Width: 100, Height: 8}, Text: "Heading", Wrap: TextWrapNone})
	found := false
