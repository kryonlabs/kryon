...
static void
ui_draw_bottom_nav_icon(Texture2D icon, Rectangle dst, Color tint)
{
    Rectangle src;
...
DrawUIBottomNav(BottomNavProps nav)
{
    ...
    for(i = 0; i < count; i++) {
        ...
        Color icon_tint = (Color){255, 255, 255, icon_alpha};

        if(ui_material_style()) {
            Rectangle item_bounds = {(float)x, (float)y, (float)w, (float)height};
            int label_font = Text12;
            ...
            ui_draw_bottom_nav_icon(item->icon, dst, icon_tint);
