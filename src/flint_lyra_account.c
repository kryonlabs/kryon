...
#define FLINT_LYRA_ACCOUNT_KEY_HEADER "lyra-account-key-v1"
#define FLINT_LYRA_LEGACY_UKU_KEY_HEADER "account-key-v1"
#define FLINT_LYRA_LEGACY_INBE_KEY_HEADER "inbe-sync-key-v1"

typedef struct FlintSha256Ctx {
    uint32_t state[8];
    uint64_t bit_len;
    uint8_t data[64];
    size_t data_len;
...
static void
sha256_transform(FlintSha256Ctx *ctx, const uint8_t data[64])
{
    uint32_t m[64];
...
static void
sha256_init(FlintSha256Ctx *ctx)
{
...
static void
sha256_update(FlintSha256Ctx *ctx, const uint8_t *data, size_t len)
{
...
static void
sha256_final(FlintSha256Ctx *ctx, uint8_t hash[32])
{
...
static void
parse_account_line(FlintLyraAccount *account, const char *line, size_t line_len)
{
...
void
flint_lyra_sha256_hex(const uint8_t *data, size_t len, char out_hex[FLINT_LYRA_PUBLIC_ID_HEX_SIZE])
{
    uint8_t digest[32];
    FlintSha256Ctx sha;
    ...
    bytes_to_hex(digest, sizeof(digest), out_hex, FLINT_LYRA_PUBLIC_ID_HEX_SIZE);
}

int
flint_lyra_account_available(void)
{
#if defined(FLINT_HAS_LIBOQS)
    return 1;
#else
...
int
flint_lyra_account_has_values(const FlintLyraAccount *account)
{
...
int
flint_lyra_account_validate(FlintLyraAccount *account)
{
    uint8_t public_key[1312];
    char expected_public_id[FLINT_LYRA_PUBLIC_ID_HEX_SIZE];
    ...
    if(!hex_to_bytes(account->public_key_hex, public_key, sizeof(public_key)))
        return 0;
    flint_lyra_sha256_hex(public_key, sizeof(public_key), expected_public_id);
...
int
flint_lyra_account_parse_text(const char *text, FlintLyraAccount *account)
{
    const char *line;
    const char *next;
    ...
    return flint_lyra_account_validate(account);
}

int
flint_lyra_account_export_text(const FlintLyraAccount *account, char *out, size_t out_size)
{
    int len;

    if(!flint_lyra_account_has_values(account) || out == NULL || out_size == 0)
        return 0;
    len = snprintf(out, out_size,
                   FLINT_LYRA_ACCOUNT_KEY_HEADER
                   "\nalgorithm=ML-DSA-44\npublic_id=%s\npublic_key=%s\nprivate_key=%s\n",
...
int
flint_lyra_account_import_file(const char *filename, FlintLyraAccount *account)
{
    ...
    if(body == NULL)
        return 0;
    ok = flint_lyra_account_parse_text(body, account);
    free(body);
    return ok;
}

int
flint_lyra_account_export_file(const FlintLyraAccount *account, const char *filename)
{
    ...
    if(filename == NULL || filename[0] == '\0' ||
       !flint_lyra_account_export_text(account, body, sizeof(body)))
        return 0;
...
int
flint_lyra_account_create(FlintLyraAccount *account)
{
#if defined(FLINT_HAS_LIBOQS)
    OQS_SIG *sig;
    uint8_t public_key[1312];
    uint8_t private_key[2560];
    FlintLyraAccount generated;
    ...
    flint_lyra_sha256_hex(public_key, sizeof(public_key), generated.public_id);
...
int
flint_lyra_account_sign_hex(const FlintLyraAccount *account, const uint8_t *message,
                            size_t message_len, char *out_signature_hex, size_t out_size)
{
    ...
    if(!flint_lyra_account_has_values(account) || out_signature_hex == NULL ||
