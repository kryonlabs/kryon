...
$(BUILD_DIR)/c/composition_queue.h: src/backend/composition_queue.zi $(BUILD_DIR)/ziran-toolchain.stamp
	mkdir -p $(BUILD_DIR)/c
	$(ZI2C_BIN) --no-main --strict --root src/backend \
		-o $(BUILD_DIR)/c src/backend/composition_queue.zi

$(BUILD_DIR)/libkryon_host.a: src/backend/frame_pacing_host.c src/backend/cursor_host.c src/backend/raster_host.c src/backend/font_metrics_host.c src/backend/image_host.c src/backend/image_software.c src/backend/kss_string_host.c src/backend/composition_host.c $(BUILD_DIR)/c/kryon_portable_host.h $(BUILD_DIR)/c/composition_queue.h $(ZIRAN_INCLUDE)/ziran_host.h Makefile
	mkdir -p $(BUILD_DIR)
	...
	rm -f $@
	$(AR) rcs $@ $(BUILD_DIR)/frame_pacing_host.o $(BUILD_DIR)/cursor_host.o $(BUILD_DIR)/raster_host.o $(BUILD_DIR)/font_metrics_host.o $(BUILD_DIR)/image_host.o $(BUILD_DIR)/image_software.o $(BUILD_DIR)/kss_string_host.o $(BUILD_DIR)/composition_host.o $(BUILD_DIR)/composition_queue.o
