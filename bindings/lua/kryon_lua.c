...
static int lua_kryon_canvas_create(lua_State *L) {
    uint16_t width = luaL_checkinteger(L, 1);
    uint16_t height = luaL_checkinteger(L, 2);
    uint32_t background_color = luaL_optinteger(L, 3, 0x000000FF);

    // Create canvas component
    kryon_component_t* component = kryon_canvas_create(width, height, background_color, NULL, NULL);
    if (!component) {
        return luaL_error(L, "Failed to create canvas component");
    }
