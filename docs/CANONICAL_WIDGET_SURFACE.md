...
| `src/ui/composition_input.zi`, `src/backend/composition_queue.zi` | Normal Ziran import and Ziran queue for raw composition events; the portable test binds a Ziran provider, while production platform adapters still need to submit events | checked Ziran plus platform host |
