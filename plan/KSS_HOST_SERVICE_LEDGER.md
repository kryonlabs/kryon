...
| Public C style API | `include/ui_style_pack_props.generated.h` | host service boundary | Declare retained C API for style registration, activation, resolving, and diagnostics; generated from `runtime/style_pack_props.kry`. | Generated parser/cascade modules plus C source-retaining registry. | `make public-headers-compile-check style-pack-source-test` |
| Embedded runtime declarations | `cmd/kir/runtime_declarations.generated.h` | compiler-emitted host glue | Mirrors `runtime/*_props.kry` sources into the kir front end; contains the style-pack signatures by construction. | The props modules own the declarations. | `make runtime-declarations-check` |
| Kry parser/formatter wrapper | `src/ui/kss_parser.kry` | host service | Allocate parser state, copy diagnostics/source traces, resolve/import source text supplied by host, and expose C wrapper entry points. | `runtime/kss_parser.kry`, `runtime/kss_formatter.kry`. | `make kss-parser-test kss-matched-test kss-formatter-test` |
| Kry active style resolver | `src/ui/style_sheet.kry` | UI policy | Hold the active style pack and route render-time queries to the generated cascade. | `runtime/style_sheet.kry`. | `make style-sheet-policy-test style-pack-source-test style-release-table-repro-test style-release-table-emitter-test style-release-startup-test` |
| Kry source-retaining pack registry | `src/ui/style_pack_source.kry` | host service | Retain pack source strings, provide module imports, enumerate variants, and atomically update active packs. | `runtime/kss_parser.kry`, `runtime/style_sheet.kry`. | `make style-pack-source-test style-release-table-repro-test style-builtins-test` |
...
| C release style table generator | `scripts/generate-c-style-tables.py` | generated-data tool | Compile authored KSS/import/theme/variant inputs through the shared parser at build time and emit typed C `StyleRule` table artifacts under `build/.../generated/src`. | `runtime/kss_parser.kry`, `runtime/style_sheet.kry`, authored KSS files. | `make style-release-table-emitter-test style-release-table-import-emitter-test style-release-startup-test` |
