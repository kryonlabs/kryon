...
modal and Kryon carries it into the next frame. Until the current frame registers the
modal bounds again, Kryon captures all pointer input so controls underneath the modal
...
button is clicked; Kryon should decide whether pointer input is captured.
...
Before this call on a frame following an active modal, Kryon captures all pointer input.
After this call, normal Kryon controls inside the rectangle remain clickable because
...
## What To Move Into Kryon

Move behavior into Kryon when it is general UI behavior rather than product behavior:
...
Kryon owns desktop file dialog backend selection. Applications should call
...
Applications should vendor Kryon as a git submodule. Do not edit
`vendor/kryon` directly for permanent changes.

Use this flow:

1. Edit the real Kryon repository.
2. Commit and push Kryon.
3. In the downstream app, update the `vendor/kryon` submodule to the pushed commit.
