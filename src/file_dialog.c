#include "file_dialog_internal.h"
...
static char g_dialog_theme_scope[THEME_NAME_SIZE] = "";
...
static Color dialog_theme_get(const char *key) {
    if(g_dialog_theme_scope[0] == '\0')
        return GetCurrentThemeColor(key);
    return GetThemeColor(g_dialog_theme_scope, key);
}

static void dialog_apply_theme(void) {
    SetUIColors(dialog_theme_get("text"),
...
SetFileDialogCurrentDir(FileDialog *dlg, const char *path)
{
    ...
    internal = (FileDialogInternal *)dlg->_internal;
    if(internal == NULL)
        return 0;
...
static FileDialogInternal *create_internal(void) {
    FileDialogInternal *internal = calloc(1, sizeof(FileDialogInternal));
    if(internal != NULL) {
        internal->hover_index = -1;
        internal->last_clicked_index = -1;
    }
...
static int should_show_entry(FileDialogInternal *internal, const char *name, int *is_dir, long *size) {
...
static void render_header(FileDialog *dlg, Rectangle dialog_rect) {
    ...
    int title_x = dialog_rect.x + ScaleUIPx(16);
    int title_y = dialog_rect.y + ScaleUIPx(12);

    DrawUIText(title, title_x, title_y, title_font, title_color);
}

static void render_breadcrumb(FileDialog *dlg, Rectangle dialog_rect) {
    FileDialogInternal *internal = (FileDialogInternal *)dlg->_internal;

    int breadcrumb_y = dialog_rect.y + ScaleUIPx(44);
    int breadcrumb_x = dialog_rect.x + ScaleUIPx(16);
    int breadcrumb_height = ScaleUIPx(30);
    int up_w = ScaleUIPx(44);

    internal->breadcrumb_rect = (Rectangle){
        breadcrumb_x + up_w + ScaleUIPx(8), breadcrumb_y,
        dialog_rect.width - ScaleUIPx(40) - up_w, breadcrumb_height
    };

    if(DrawUIGenericButton(breadcrumb_x, breadcrumb_y, up_w, breadcrumb_height,
    ...
    Color border = DarkenUIColor(bg, 30);
    ...
    int text_x = (int)internal->breadcrumb_rect.x + ScaleUIPx(8);
    int text_y = breadcrumb_y + ScaleUIPx(7);
    int max_text_width = (int)internal->breadcrumb_rect.width - ScaleUIPx(16);
    while(display_path[0] != '\0' && MeasureUIText(display_path, font) > max_text_width) {
    ...
    DrawUIText(display_path, text_x, text_y, font, text);
}

static void render_file_list(FileDialog *dlg, Rectangle dialog_rect) {
    FileDialogInternal *internal = (FileDialogInternal *)dlg->_internal;

    int list_y = dialog_rect.y + ScaleUIPx(84);
    int list_x = dialog_rect.x + ScaleUIPx(16);
    int scrollbar_w = ScaleUIPx(12);
    int list_width = dialog_rect.width - ScaleUIPx(32);
    int bottom_y = dialog_rect.y + internal->dialog_height - ScaleUIPx(112);
    ...
    Color bg = LightenUIColor(dialog_theme_get("background"), 10);
    DrawRectangleRec(internal->file_list_rect, bg);

    Color border = DarkenUIColor(dialog_theme_get("background"), 30);
    DrawRectangleLinesEx(internal->file_list_rect, 1, border);

    BeginUIClip(list_x, list_y, list_width, list_height);

    int font = GetUISmallFontSize();
    Color text = dialog_theme_get("text");
    Color hover = dialog_theme_get("button_hover");
    Color selected = DarkenUIColor(dialog_theme_get("button"), 20);
    ...
    for(int i = start_idx; i < end_idx; i++) {
        ...
        int max_name_width = list_width - scrollbar_w - ScaleUIPx(16);
        while(display_name[0] != '\0' && MeasureUIText(display_name, font) > max_name_width) {
        ...
        DrawUIText(display_name, list_x + ScaleUIPx(8), item_y + ScaleUIPx(4), font, text);
    }
...
static void render_scrollbar(FileDialog *dlg, Rectangle dialog_rect) {
    ...
    int scrollbar_x = internal->file_list_rect.x + internal->file_list_rect.width - ScaleUIPx(12);
    ...
    Color bg = DarkenUIColor(dialog_theme_get("background"), 40);
    ...
    if(thumb_height < ScaleUIPx(24))
        thumb_height = ScaleUIPx(24);
...
static void render_filename_input(FileDialog *dlg, Rectangle dialog_rect) {
    FileDialogInternal *internal = (FileDialogInternal *)dlg->_internal;
    if(dlg->mode == FILE_DIALOG_SELECT_FOLDER)
        return;

    int input_y = dialog_rect.y + internal->dialog_height - ScaleUIPx(96);
    int input_x = dialog_rect.x + ScaleUIPx(16);
    int font = GetUISmallFontSize();
    int label_width = MeasureUIText("Filename:", font) + ScaleUIPx(8);
    int input_width = dialog_rect.width - ScaleUIPx(32) - label_width;
    int input_height = ScaleUIPx(24);
    if(input_width < ScaleUIPx(24))
        input_width = ScaleUIPx(24);
    ...
    Color text = dialog_theme_get("text");
    Color label_dim = DarkenUIColor(text, 40);
    DrawUIText("Filename:", input_x, GetUIControlTextY("Filename:", input_y, input_height, font), font, label_dim);

    char display_filename[256];
    snprintf(display_filename, sizeof(display_filename), "%s", internal->filename_input);
    int max_filename_width = input_width - ScaleUIPx(8);
    while(display_filename[0] != '\0' && MeasureUIText(display_filename, font) > max_filename_width)
        memmove(display_filename, display_filename + 1, strlen(display_filename));

    DrawUIText(display_filename, (int)internal->filename_rect.x + ScaleUIPx(4),
            input_y + ScaleUIPx(4), font, text);
    ...
    if(internal->focus_area == 2) {
        ...
        if(internal->cursor_visible) {
            int cursor_x = (int)internal->filename_rect.x + ScaleUIPx(4) +
                          MeasureUIText(display_filename, font);
            DrawLine(cursor_x, input_y + ScaleUIPx(4),
                    cursor_x, input_y + input_height - ScaleUIPx(4), text);
        }
    } else {
...
static void render_buttons(FileDialog *dlg, Rectangle dialog_rect) {
    ...
    int button_y = dialog_rect.y + internal->dialog_height - ScaleUIPx(58);
    int button_width = ScaleUIPx(96);
    int button_height = ScaleUIPx(30);
    int show_hidden = internal->show_hidden_files;
    const char *ok_label = dlg->mode == FILE_DIALOG_SELECT_FOLDER ? "Choose" : "OK";

    internal->button_cancel_rect = (Rectangle){
        dialog_rect.x + dialog_rect.width - ScaleUIPx(212),
        button_y, button_width, button_height
    };

    internal->button_ok_rect = (Rectangle){
        dialog_rect.x + dialog_rect.width - ScaleUIPx(108),
        button_y, button_width, button_height
    };

    if(DrawUIGenericButton((int)internal->button_cancel_rect.x,
    ...
    if(DrawUIGenericButton((int)internal->button_ok_rect.x,
    ...
    Color text = dialog_theme_get("text");
    int checkbox_size = ScaleUIPx(16);
    int checkbox_x = dialog_rect.x + ScaleUIPx(16);
    int checkbox_y = button_y + ScaleUIPx(6);

    internal->show_hidden_checkbox_rect = (Rectangle){
        checkbox_x, checkbox_y, checkbox_size, checkbox_size
    };

    if(DrawUICheckboxToggle(checkbox_x, checkbox_y, "Show hidden files", &show_hidden) &&
    ...
    if(dlg->mode == FILE_DIALOG_SELECT_FOLDER) {
        ...
        DrawUIText(folder_label, checkbox_x, dialog_rect.y + internal->dialog_height - ScaleUIPx(92),
                        GetUISmallFontSize(), text);
    }
}
...
BeginSelectFileDialogFolder(FileDialog *dlg, const char *title)
{
    ...
    if(!dlg || !title) return;
    InitFileDialog(dlg);
    dlg->mode = FILE_DIALOG_SELECT_FOLDER;
    copy_text(dlg->title, sizeof(dlg->title), title);
    dlg->active = 1;
    dlg->confirmed = 0;
...
int SaveFileDialog(FileDialog *dlg, const char *title, const char *default_filename) {
    ...
    BeginSaveFileDialog(dlg, title, default_filename);
    return run_dialog(dlg);
}
...
int SelectFileDialogFolder(FileDialog *dlg, const char *title) {
    ...
    BeginSelectFileDialogFolder(dlg, title);
    run_dialog(dlg);
