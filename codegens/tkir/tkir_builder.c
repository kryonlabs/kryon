/**
 * @file tkir_builder.c
 * @brief TKIR Builder Implementation
 *
 * Implementation of KIR to TKIR transformation.
 *
...
static TKIRPackOptions* tkir_compute_pack_options_with_alignment(
...
TKIRBuilderContext* tkir_builder_create(TKIRRoot* root) {
    if (!root) {
        tkir_error("Cannot create builder context with NULL root");
        return NULL;
    }

    TKIRBuilderContext* ctx = calloc(1, sizeof(TKIRBuilderContext));
    if (!ctx) {
        tkir_error("Failed to allocate builder context");
        return NULL;
    }
    ...
    if (!ctx->handlers) {
        free(ctx);
        tkir_error("Failed to create handler tracker");
        return NULL;
    }
...
TKIRRoot* tkir_build_from_kir(const char* kir_json, bool verbose) {
    ...
    // Build from cJSON
    TKIRRoot* root = tkir_build_from_cJSON(kir_root, verbose);
    if (!root) {
...
TKIRRoot* tkir_build_from_cJSON(cJSON* kir_root, bool verbose) {
    ...
    if (!root) {
        tkir_error("Failed to create TKIR root");
        return NULL;
    }

    // Create builder context
    TKIRBuilderContext* ctx = tkir_builder_create(root);
    if (!ctx) {
        tkir_root_free(root);
        return NULL;
    }
    ...
    if (!tkir_builder_transform_root(ctx, kir_root)) {
        tkir_error("Failed to transform KIR to TKIR");
        tkir_builder_free(ctx);
        tkir_root_free(root);
        return NULL;
    }
    ...
    if (logic_block) {
        tkir_builder_extract_data_bindings(ctx, logic_block);
    }
...
bool tkir_builder_transform_root(TKIRBuilderContext* ctx, cJSON* kir_root) {
    if (!ctx || !kir_root) {
        tkir_error("Invalid arguments to transform_root");
        return false;
    }
    ...
    TKIRWidget* root_widget = tkir_builder_transform_component(
        ctx, root_component, NULL, ctx->root_background, NULL, 0, 1);
    if (!root_widget) {
        tkir_error("Failed to transform root component");
        return false;
    }

    // Serialize root widget to JSON and add to widgets array
    cJSON* widget_json = tkir_widget_to_json(root_widget);
    if (!widget_json) {
        tkir_widget_free(root_widget);
        return false;
    }
...
TKIRWidget* tkir_builder_transform_component(TKIRBuilderContext* ctx,
    ...
    TKIRWidget* widget = tkir_widget_create(component, parent_id, &ctx->widget_id_counter);
    if (!widget) {
        return NULL;
    }

    // Resolve background color (with inheritance)
    widget->background = tkir_resolve_background(component, parent_bg);

    // Normalize properties
    cJSON* width = cJSON_GetObjectItem(component, "width");
    if (width) {
        widget->width = tkir_normalize_size(width);
    }

    cJSON* height = cJSON_GetObjectItem(component, "height");
    if (height) {
        widget->height = tkir_normalize_size(height);
    }

    widget->font = tkir_normalize_font(component);
    widget->border = tkir_normalize_border(component);
    ...
    if (parent_id && parent_component) {
        ...
        widget->layout = tkir_builder_compute_layout_with_alignment(
            ctx, widget, component, parent_type, main_align, cross_align,
            child_index, total_children);
    }

    // Extract event handlers
    tkir_builder_extract_handlers(ctx, widget, component);
    ...
    if (children && cJSON_IsArray(children)) {
        ...
        cJSON_ArrayForEach(child, children) {
            ...
            TKIRWidget* child_widget = tkir_builder_transform_component(
                ctx, child, widget->id, widget->background, component, idx, total);

            if (child_widget) {
                // Serialize child widget to JSON
                cJSON* child_json = tkir_widget_to_json(child_widget);
                if (child_json) {
...
TKIRSize* tkir_normalize_size(cJSON* value) {
    ...
    TKIRSize* size = calloc(1, sizeof(TKIRSize));
    if (!size) return NULL;
...
TKIRFont* tkir_normalize_font(cJSON* component) {
    ...
    TKIRFont* tkir_font = calloc(1, sizeof(TKIRFont));
    if (!tkir_font) return NULL;

    // Font family
    if (cJSON_IsString(font)) {
        tkir_font->family = font->valuestring;
        tkir_font->size = 12.0;
        tkir_font->size_unit = "px";
    } else if (cJSON_IsObject(font)) {
        cJSON* family = cJSON_GetObjectItem(font, "family");
        tkir_font->family = (family && cJSON_IsString(family)) ? family->valuestring : NULL;

        cJSON* size = cJSON_GetObjectItem(font, "size");
        if (size) {
            if (cJSON_IsNumber(size)) {
                tkir_font->size = size->valuedouble;
                tkir_font->size_unit = "px";
            } else if (cJSON_IsString(size)) {
                sscanf(size->valuestring, "%lf", &tkir_font->size);
                tkir_font->size_unit = "px";  // TODO: Parse unit from string
            }
        }

        cJSON* weight = cJSON_GetObjectItem(font, "weight");
        tkir_font->weight = (weight && cJSON_IsString(weight)) ? weight->valuestring : NULL;

        cJSON* style = cJSON_GetObjectItem(font, "style");
        tkir_font->style = (style && cJSON_IsString(style)) ? style->valuestring : NULL;
    } else {
        free(tkir_font);
        return NULL;
    }
...
TKIRBorder* tkir_normalize_border(cJSON* component) {
    ...
    TKIRBorder* tkir_border = calloc(1, sizeof(TKIRBorder));
    if (!tkir_border) return NULL;

    if (cJSON_IsString(border)) {
        tkir_border->color = border->valuestring;
        tkir_border->width = 1;
        tkir_border->style = "solid";
    } else if (cJSON_IsObject(border)) {
        cJSON* width = cJSON_GetObjectItem(border, "width");
        tkir_border->width = (width && cJSON_IsNumber(width)) ? width->valueint : 1;

        cJSON* color = cJSON_GetObjectItem(border, "color");
        tkir_border->color = (color && cJSON_IsString(color)) ? color->valuestring : NULL;

        cJSON* style = cJSON_GetObjectItem(border, "style");
        tkir_border->style = (style && cJSON_IsString(style)) ? style->valuestring : "solid";
    } else {
        free(tkir_border);
        return NULL;
    }
...
TKIRLayoutOptions* tkir_builder_compute_layout(TKIRBuilderContext* ctx,
    ...
    TKIRLayoutOptions* layout = calloc(1, sizeof(TKIRLayoutOptions));
    if (!layout) return NULL;

    switch (layout_type) {
        case CODEGEN_LAYOUT_PACK:
            layout->type = TKIR_LAYOUT_PACK;
            layout->pack = *tkir_compute_pack_options(component, parent_type, child_index, total_children);
            break;

        case CODEGEN_LAYOUT_GRID:
            layout->type = TKIR_LAYOUT_GRID;
            layout->grid = *tkir_compute_grid_options(component);
            break;

        case CODEGEN_LAYOUT_PLACE:
            layout->type = TKIR_LAYOUT_PLACE;
            layout->place = *tkir_compute_place_options(component);
            break;
...
TKIRPackOptions* tkir_compute_pack_options(cJSON* component,
                                           ...
                                           int child_index,
                                           int total_children) {
    static TKIRPackOptions options;
...
TKIRGridOptions* tkir_compute_grid_options(cJSON* component) {
    static TKIRGridOptions options;
    memset(&options, 0, sizeof(options));
...
TKIRPlaceOptions* tkir_compute_place_options(cJSON* component) {
    static TKIRPlaceOptions options;
    memset(&options, 0, sizeof(options));
...
TKIRLayoutOptions* tkir_builder_compute_layout_with_alignment(
    ...
    switch (layout_type) {
        case CODEGEN_LAYOUT_PACK:
            layout->type = TKIR_LAYOUT_PACK;
            {
                TKIRPackOptions* opts = tkir_compute_pack_options_with_alignment(
        ...
        case CODEGEN_LAYOUT_GRID:
            layout->type = TKIR_LAYOUT_GRID;
            {
                TKIRGridOptions* opts = tkir_compute_grid_options(component);
        ...
        case CODEGEN_LAYOUT_PLACE:
            layout->type = TKIR_LAYOUT_PLACE;
            {
                TKIRPlaceOptions* opts = tkir_compute_place_options(component);
...
bool tkir_builder_extract_handlers(TKIRBuilderContext* ctx,
    ...
    cJSON_ArrayForEach(event, events) {
        ...
        // Create new handler
        TKIRHandler* handler = tkir_handler_create(event, widget->id, &ctx->handler_id_counter);
        if (!handler) {
            continue;
        }
        ...
        cJSON* handler_json = tkir_handler_to_json(handler);
        if (handler_json) {
...
cJSON* tkir_widget_to_json(TKIRWidget* widget) {
    ...
    cJSON_AddStringToObject(json, "tk_type", widget->tk_type);
    if (widget->kir_type) {
    ...
    if (widget->width) {
        cJSON_AddItemToObject(props, "width", tkir_size_to_json(widget->width));
    }
    if (widget->height) {
        cJSON_AddItemToObject(props, "height", tkir_size_to_json(widget->height));
    }
    if (widget->font) {
        cJSON_AddItemToObject(props, "font", tkir_font_to_json(widget->font));
    }
    if (widget->border) {
        cJSON_AddItemToObject(props, "border", tkir_border_to_json(widget->border));
    }
    ...
    // Layout
    if (widget->layout) {
        cJSON* layout = tkir_layout_to_json(widget->layout);
        if (widget->parent_id) {
...
cJSON* tkir_layout_to_json(TKIRLayoutOptions* layout) {
    ...
    cJSON_AddStringToObject(json, "type", tkir_layout_type_to_string(layout->type));

    cJSON* options = cJSON_CreateObject();

    switch (layout->type) {
        case TKIR_LAYOUT_PACK: {
