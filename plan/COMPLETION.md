...
| Web parity | `tests/generated_runtime_parity_test.sh` reports scroll_content and composed_popup as native/Go only; composed_popup now has partial JS coverage for content/tools/tooltip/modal-open behavior, while composition, drag_drop, and menus execute their generated behavior checks in JS too. | Two fixtures still need complete JS execution. Preserve the distinction between partial composed-popup coverage and full three-backend fixtures. |
...
- [ ] Execute scroll_content and composed_popup in JS using the same event
  ...
  popup content/tools/tooltip/modal-open checks now run as partial JS coverage.
