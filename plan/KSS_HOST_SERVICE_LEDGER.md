...
| C parser/formatter wrapper | `src/ui/kss_parser.c` | host service | Allocate parser state, copy diagnostics, resolve/import source text supplied by host, and expose C wrapper functions. | `runtime/kss_parser.kry`, `runtime/kss_formatter.kry`. | `make kss-parser-test kss-matched-test kss-formatter-test` |
