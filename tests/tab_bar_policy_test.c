...
main(void)
{
    ...
    TabBarTabPointerDecision pointer;
    ...
    assert(!reorder_drag.start_drag);
    assert(reorder_drag.dragging);
    reorder_release = TabBarReorderReleaseDecisionFor(0, 1, 1, 1, 1, 3);
    assert(reorder_release.finish);
    assert(reorder_release.consume_release);
    reorder_release = TabBarReorderReleaseDecisionFor(0, 1, 1, 0, 1, 3);
    assert(!reorder_release.finish);
    assert(!reorder_release.consume_release);
    pointer = TabBarTabPointerDecisionFor(1, 1, 0, 1);
    assert(pointer.close);
    assert(!pointer.activate);
    assert(pointer.consume_release);
    pointer = TabBarTabPointerDecisionFor(0, 1, 0, 1);
    assert(!pointer.close);
    assert(pointer.activate);
    assert(pointer.consume_release);
    pointer = TabBarTabPointerDecisionFor(0, 1, 1, 1);
    assert(!pointer.close);
    assert(!pointer.activate);
    assert(!pointer.consume_release);
