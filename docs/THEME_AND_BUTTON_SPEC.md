...
Historical design specification. Its C examples and gallery references describe
the former `.kry` runtime and are not current Ziran API instructions. See the
[current Ziran example](../examples/README.md) for a runnable checked Button.

Status: historical design; checked Ziran implementation differs
...
The [button gallery](../examples/button_gallery.md) is a historical audit of
the former runtime. The image above is a design illustration.
...
This table records the original migration map. The canonical native Button
implementation now lives in `src/ui/button.kry`, SegmentedControl lives in
`src/ui/segmented_control.kry`, and both use KSS style resolution. The other
rows remain historical targets or continuing migration work.
...
| `src/ui/modal.kry`, `rows.kry`, `pager.kry`, navigation | migrate remaining named styles to semantic props |
