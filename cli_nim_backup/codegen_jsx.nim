## JSX Code Generator for Kryon
## Generates JavaScript React components without type annotations from .kir files

import json, strformat, os, strutils, tables
import codegen_react_common

# =============================================================================
# JavaScript-Specific Component Generation
# =============================================================================

proc generateComponentFunction(def: ComponentDef, ctx: var ReactContext): string =
  ## Generate a JavaScript React function component (no types)
  let name = def.name

  # NO interface generation for JavaScript

  # Generate function signature WITHOUT type annotation
...
proc generateJsxFromKir*(kirPath: string): string =
  ## Generate JSX code from .kir file (JavaScript React-Hooks style)
  if not fileExists(kirPath):
    raise newException(IOError, "File not found: " & kirPath)

  let kirJson = parseFile(kirPath)

  # Build context with JavaScript mode
  ...
  result.add("// Generated by Kryon Code Generator (JavaScript React-Hooks Style)\n\n")
...
when isMainModule:
  ...
  if paramCount() < 1:
    echo "Usage: codegen_jsx <input.kir> [output.jsx]"
    quit(1)

  let inputPath = paramStr(1)
  let outputPath = if paramCount() >= 2: paramStr(2)
                   else: inputPath.changeFileExt("jsx")

  try:
    let jsxCode = generateJsxFromKir(inputPath)
    writeFile(outputPath, jsxCode)
