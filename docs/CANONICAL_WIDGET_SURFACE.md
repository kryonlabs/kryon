...
| `runtime/paned_view.kry` | PanedView split/layout/handle geometry and change policy | `.kry canonical` |
...
| `runtime/scroll.kry` | Scroll measurement, sizing, wheel, content/thumb drag offset, scrollbar drag decision, and ensure-visible policy | `.kry canonical` |
...
| `runtime/toast.kry` | Toast duration, layout, text-placement, and truncation policy | `.kry canonical` |
...
| `runtime/modal.kry` | Modal layout, frame geometry, message line-gap, outside-dismissal, and action policy | `.kry canonical` |
...
| Layout | `Column`/`Row`/`Stack` content and child placement policy, `Group` bounds/content policy, `Screen` viewport fallback bounds policy, `Grid`, `Fieldset` layout policy, `PanedView` split/layout/change geometry, `Collapsible` header geometry, `Separator`, `Scroll` measurement/sizing/wheel/content-drag/thumb-drag/scrollbar-drag decision/ensure-visible/clip geometry policy, shared `Surface`/`Style`/`Material` policy, `Reorder` metrics/handle geometry/placeholder paint geometry/target-index/result policy, `ReorderState`/`ReorderItem`/`ReorderList`/`ReorderListResult` generated support records | scroll/list/table begin-end wrappers; scroll content pointer ownership and reorder pointer ownership/gesture lifecycle remain host support |
...
| `PanedView` | `UI/Layout` | Split panes | `runtime/paned_view.kry` | Partly `.kry-backed` | Split clamp, layout, handle geometry, pointer split, and change policy are `.kry`; host keeps drag/input ownership. |
...
| `Modal` | `UI/Overlays` | Dialog | `runtime/modal.kry` | Partly `.kry-backed` | Layout, frame geometry, message line-gap, outside-dismissal, and action sizing policy are `.kry`; host keeps capture application, release consumption, text editing, and drawing. |
...
| `PanedView` | `.kry canonical` | Public props live in `runtime/paned_view_props.kry`; split clamp, layout, handle geometry, pointer split, and change policy are in `.kry`; host keeps drag/input ownership. |
...
| `Scroll` | `.kry canonical` | Public props live in `runtime/scroll_props.kry`; lexical scroll-content block. Measurement, sizing, wheel offset, content/thumb drag offset, scrollbar drag decision, ensure-visible policy, and clip/visual bounds geometry are in `.kry`; host keeps content pointer ownership, clip-stack application, and lowered scope ownership. |
...
| `Modal` | `.kry canonical` | Layout, frame geometry, outside-dismissal, and action sizing policy are in `.kry`; title, message, and action text typography is KSS-owned with resolved font sizes used directly; prompt fields use `TextField` typography; host handles capture application, release consumption, text editing, and drawing. |
| `Toast` | `.kry canonical` | Public toast feedback surface. Duration, layout, text-placement, and truncation policy are in `.kry`; host keeps message storage, timing source, text measurement, and drawing. |
