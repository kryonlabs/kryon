...
$(BUILD_DIR)/libkryon_host.a: src/backend/frame_pacing_host.c src/backend/raster_host.c include/kryon_portable_host.h $(ZIRAN_INCLUDE)/ziran_host.h Makefile
	mkdir -p $(BUILD_DIR)
	...
	rm -f $@
	$(AR) rcs $@ $(BUILD_DIR)/frame_pacing_host.o $(BUILD_DIR)/raster_host.o
