                 ...
                 uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
    ...
    assert(x >= 10 && y >= 20 && x + w <= 210 && y + h <= 120);
    fills++;
}
                 ...
                 uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
    ...
    assert(0 && "toast labels must be clipped");
}
                    ...
                    uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
    ...
    assert(font == 14 && x >= 10 && y >= 20);
    assert(clip[0] >= 10 && clip[1] >= 20 &&
           clip[0] + clip[2] <= 210 && clip[1] + clip[3] <= 120);
    if (labels == 0 || labels == 1) {
        assert(bytes == 5 && memcmp(value, "Hello", 5) == 0);
    } else if (labels == 2) {
        assert(bytes == 14 &&
               memcmp(value, "12345678901234", 14) == 0);
    } else if (labels == 3) {
        assert(bytes == 3 && memcmp(value, "...", 3) == 0);
    } else {
        assert(labels == 4 && bytes == 1 && value[0] == 'X');
    }
    labels++;
}
