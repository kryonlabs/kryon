...
| `TextArea` | `Collections` | Text area | `src/ui/text_input.zi`, `src/ui/text_input_props.zi`, `src/ui/text_area_widget.zi`, `src/ui/syntax.zi` | checked Ziran | Caller owned text and state, visual wrapping, pointer selection, vertical/page navigation, scrolling, edit and clipboard intents, composition event state and commit edits, KSS, Ziran/C/Make syntax coloring, composition underlines, and clipped paint are checked. Preedit display, platform event routing, clipboard IO, and native host integration remain. |
...
| `TextArea` | checked Ziran | Caller owned text and composition state; wrapped rows, pointer selection, page navigation, scroll, KSS and syntax paint, composition underlines, multiline edit intents, copy/cut/paste decisions, and composition event state and commit edits. Preedit display, platform event routing, clipboard IO, and native host integration remain. |
...
| `TextArea` | checked Ziran | `text_area_widget.zi` and `syntax.zi` own visual rows, retained input, selection, scroll, KSS and syntax paint, composition underlines, multiline edit intents, copy/cut/paste decisions, and composition event state and commit edits; applications own text, composition bytes, and focus state. Preedit display, platform event routing, clipboard IO, and native host integration remain. |
