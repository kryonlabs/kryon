...
Build :: (reversed: bool, second_disabled: bool,
    ...
    if reversed {
        second: s32 = TreeSubmit(cast(u64)12, 0, WidgetKindButton,
            Rectangle.{20.0, 20.0, 30.0, 30.0})
        TreeSetInteractive(second, second_disabled, false, 0)
    }
    if first_present {
        first: s32 = TreeSubmit(cast(u64)11, 0, WidgetKindButton,
...
Answer :: () -> s32 {
    TreeStart(cast(u64)1, Rectangle.{0.0, 0.0, 100.0, 100.0})
    TreeSubmit(cast(u64)99, 0, WidgetKindButton,
        Rectangle.{10.0, 10.0, 30.0, 30.0})
    if !TreeFinish() || TreeHitAt(15.0, 15.0) != -1 { return -20 }
    if !Build(false, false, true) || TreeCount() != 3 { return -1 }
    first: s32 = TreeFind(cast(u64)11, cast(u64)1, WidgetKindButton)
    second: s32 = TreeFind(cast(u64)12, cast(u64)1, WidgetKindButton)
    if first != 1 || second != 2 || TreeHitAt(25.0, 25.0) != second ||
        TreeHitAt(100.0, 25.0) != -1 { return -2 }
    identity: s32 = TreeNodeAt(second).identity_generation
    ...
    second = TreeFind(cast(u64)12, cast(u64)1, WidgetKindButton)
    first = TreeFind(cast(u64)11, cast(u64)1, WidgetKindButton)
    if second != 1 || first != 2 ||
    ...
    TreePointerUpdate(PointerFrame.{25.0, 25.0, false, false, true})
    if TreeTakeActivationAt(TreeFind(cast(u64)12, cast(u64)1,
        WidgetKindButton)) { return -16 }

    TreeStart(cast(u64)1, Rectangle.{0.0, 0.0, 100.0, 100.0})
    card: s32 = TreeSubmit(cast(u64)13, 0, WidgetKindCard,
        Rectangle.{10.0, 10.0, 30.0, 30.0})
    TreeSetInteractive(card, false, false, 0)
    if !TreeFinish() || TreeHitAt(15.0, 15.0) != -1 { return -17 }
    TreeStart(cast(u64)1, Rectangle.{0.0, 0.0, 100.0, 100.0})
    card = TreeSubmit(cast(u64)13, 0, WidgetKindCard,
        Rectangle.{10.0, 10.0, 30.0, 30.0})
    ...
    TreeStart(cast(u64)1, Rectangle.{0.0, 0.0, 100.0, 100.0})
    checkbox: s32 = TreeSubmit(cast(u64)14, 0, WidgetKindCheckbox,
        Rectangle.{10.0, 10.0, 30.0, 30.0})
    TreeSetInteractive(checkbox, false, false, 0)
    toggle: s32 = TreeSubmit(cast(u64)15, 0, WidgetKindToggle,
    ...
    TreeStart(cast(u64)1, Rectangle.{0.0, 0.0, 100.0, 100.0})
    checkbox = TreeSubmit(cast(u64)14, 0, WidgetKindCheckbox,
        Rectangle.{10.0, 10.0, 30.0, 30.0})
    ...
    TreeStart(cast(u64)1, Rectangle.{0.0, 0.0, 100.0, 100.0})
    slider: s32 = TreeSubmit(cast(u64)16, 0, WidgetKindSlider,
    ...
    TreeStart(cast(u64)1, Rectangle.{0.0, 0.0, 100.0, 100.0})
    TreeSubmit(cast(u64)17, 0, WidgetKindText,
        Rectangle.{0.0, 0.0, 5.0, 5.0})
    slider = TreeSubmit(cast(u64)16, 0, WidgetKindSlider,
        Rectangle.{10.0, 10.0, 60.0, 20.0})
    TreeSetInteractive(slider, false, false, 0)
