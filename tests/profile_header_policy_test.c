...
main(void)
{
    ...
    assert(!action.activate);
    assert(!action.consume_release);
    action = ProfilePointerActionFromInput(true, false, true, false);
    assert(action.active);
    assert(!action.activated);
    assert(action.mark_clickable);
    assert(!action.consume_release);
    action = ProfilePointerActionFromInput(true, false, true, true);
    assert(action.active);
    assert(action.activated);
    assert(action.consume_release);
    action = ProfilePointerActionFromInput(true, true, true, true);
    assert(!action.active);
    assert(!action.activated);
    assert(!action.consume_release);
    action = ProfilePointerActionFromInput(true, false, false, true);
    assert(!action.active);
    assert(!action.activated);
    assert(!action.consume_release);
