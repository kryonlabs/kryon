...
func TestFrameOpsRecordRenderableNativeFrame(t *testing.T) {
	...
	rt.Text(TextProps{Bounds: NewRectangle(0, 0, 0, 0), Text: "geld", Wrap: TextWrapNone})
...
`, "Page Metadata", "") || !SetActiveStylePack("test.page.metadata") {
	...
	rt.Text(TextProps{Bounds: NewRectangle(0, 0, 0, 0), Text: "B", Wrap: TextWrapNone})
	rt.End()
	rt.EndFrame()
...
func TestParagraphColorUsesParagraphStyleSheet(t *testing.T) {
	...
	t.Cleanup(ClearStylePacks)
	if !RegisterStylePackSource(`
@pack test.paragraph_text_color;
tokens {
  color { ink: #314253; }
}
ParagraphText.note { foreground: ink; font-size: 18; opacity: 0.5; }
`, "Test Paragraph Text Color", "") || !SetActiveStylePack("test.paragraph_text_color") {
		t.Fatal("test paragraph color style did not activate")
	}
	rt := New(AppConfig{Width: 240, Height: 120}).(*runtime)
	y := int32(10)

	rt.Paragraph(ParagraphSpec{
		Text:      "Body",
		Width:     120,
		ClassName: StyleClassID("note"),
	}, 8, &y)

	for _, op := range rt.FrameOps() {
		if op.Kind == FrameOpText && op.Text == "Body" {
			if op.Color != (Color{R: 0x31, G: 0x42, B: 0x53, A: 0x7f}) ||
				op.FontSize != 18 {
				t.Fatalf("paragraph styled text op = %+v", op)
			}
			return
		}
	}
...
func TestFrameOpsResetEachFrame(t *testing.T) {
	...
	rt.BeginFrame()
	rt.Text(TextProps{Bounds: NewRectangle(10, 10, 0, 0), Text: "first", Wrap: TextWrapNone})
	rt.EndFrame()
...
func TestRenderCurrentFramePaintsNativeOps(t *testing.T) {
	...
	Text(TextProps{Bounds: NewRectangle(12, 12, 0, 0), Text: "geld", Wrap: TextWrapNone})
...
func TestTakeScreenshotWritesCurrentFramePNG(t *testing.T) {
	...
	Text(TextProps{Bounds: NewRectangle(4, 4, 0, 0), Text: "shot", Wrap: TextWrapNone})
	EndFrame()
