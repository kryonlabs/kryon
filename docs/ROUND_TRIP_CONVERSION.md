# Kryon Round-Trip C ↔ KIR Conversion

## Overview

Kryon supports bidirectional conversion between C source code and KIR (Kryon Intermediate Representation) format, enabling perfect round-trip transformations where `main.c` → `main.kir` → `generated_main.c` produces functionally equivalent code.
