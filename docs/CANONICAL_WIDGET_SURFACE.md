...
| `src/ui/tab_bar.zi`, `src/ui/tab_bar_widget.zi` | Checked TabBar sizing, scroll, keyboard selection, retained pointer activation, close, double click, reorder intents, KSS paint, and borrowed image values | checked Ziran |
| `src/ui/tab_bar_props.zi` | Portable TabBar props, borrowed tabs, raw input samples, and caller owned state/results | checked Ziran |
...
| `src/ui/text_input.zi`, `src/ui/text_field_widget.zi`, `src/ui/text_area_widget.zi`, `src/ui/syntax.zi` | Checked text policy, portable byte-buffer edits, UTF-8 boundaries, retained TextField and TextArea composition, edit and clipboard intents, composition event state and commit edits, selection, scroll, syntax coloring, and preedit paint in both controls | checked Ziran |
| `src/ui/composition_input.zi`, `src/backend/composition_queue.zi`, `src/backend/composition_host.c` | Normal Ziran import and Ziran queue for raw composition events; the C host converts values for the VM and platform adapters still need to submit their events | checked Ziran plus platform host |
...
| `src/ui/text_input_props.zi` | Portable TextField and TextArea props, raw input and supplied paste text, caller owned text and composition state, edit results, and clipboard write requests | checked Ziran |
...
| Navigation | `NavigationBar` item interaction and paint/configuration, checked `TabBar` composition/scroll/keyboard/pointer/close/reorder paint, `Toolbar`, `TitleBar`, and `Menu` decision policy | retained menu open/focus/input state, router/link helpers, and native host integration |
...
| `src/ui/swipe.zi`, `src/ui/tab_bar_widget.zi` | `swipe.zi`, `tab_bar_widget.zi` | TabBar composition, input, paint, and caller owned state are checked Ziran; the old tab host and store have been removed. |
...
| `TextArea` | `Collections` | Text area | `src/ui/text_input.zi`, `src/ui/text_input_props.zi`, `src/ui/text_area_widget.zi`, `src/ui/syntax.zi` | checked Ziran | Caller owned text and state, visual wrapping, pointer selection, vertical/page navigation, scrolling, edit and clipboard intents, composition event state and commit edits, KSS, Ziran/C/Make syntax coloring, preedit rows and underlines, and clipped paint are checked. Platform event routing, clipboard IO, and native host integration remain. |
...
| `TabBar` | `Navigation` | Tabs | `src/ui/tab_bar.zi`, `src/ui/tab_bar_props.zi`, `src/ui/tab_bar_widget.zi` | checked Ziran | Borrowed tabs, caller owned state, retained pointer input, keyboard selection, scrolling, close, middle/double click, reorder intents, KSS, and icon/text paint are checked. Mouse drag panning and native host integration remain. |
...
| `Modal` | `Overlays` | Dialog | `src/ui/modal.zi`, `src/ui/modal_frame_widget.zi`, `src/ui/modal_widget.zi` | frame and ActionModal checked Ziran | Frame layout, backdrop dismissal, title, message, and wrapped action buttons are checked; the old combined API and prompt input remain in the legacy host. |
...
| `TextField` | checked Ziran | Caller owned text and composition state; retained focus, cursor, selection, scroll, KSS paint, UTF-8 byte edit intents, copy/cut/paste decisions, composition event state and commit edits, and preedit paint. Platform event routing, clipboard IO, and native host integration remain. |
| `TextArea` | checked Ziran | Caller owned text and composition state; wrapped rows, pointer selection, page navigation, scroll, KSS and syntax paint, preedit rows and underlines, multiline edit intents, copy/cut/paste decisions, and composition event state and commit edits. Platform event routing, clipboard IO, and native host integration remain. |
...
| `ActionModal` | checked Ziran | Message and borrowed action values with checked wrapping, selection, and dismissal; prompt text editing remains to migrate. |
| `Modal` | migration pending | The old combined message, action, and prompt API still uses the legacy host; checked message and action composition is available as `ActionModal`. |
| `TitleBar` | `.kry canonical` | Title/action bar. |
| `TabBar` | checked Ziran | Portable tab navigation surface with borrowed tabs and caller owned state. |
...
| `TabBar` | checked Ziran | Tab bar with borrowed tabs, caller owned state, and returned actions. |
...
| `TextField` | checked Ziran | `text_field_widget.zi` owns retained input, selection, scroll, KSS, paint, byte edit intents, copy/cut/paste decisions, composition event state and commit edits, and preedit paint; applications own text, composition bytes, and focus state. Platform event routing, clipboard IO, and native host integration remain. |
| `TextArea` | checked Ziran | `text_area_widget.zi` and `syntax.zi` own visual rows, retained input, selection, scroll, KSS and syntax paint, preedit rows and underlines, multiline edit intents, copy/cut/paste decisions, and composition event state and commit edits; applications own text, composition bytes, and focus state. Platform event routing, clipboard IO, and native host integration remain. |
...
| `TabBar` | checked Ziran | `src/ui/tab_bar_widget.zi` owns layout, KSS style, input, scroll, drag, close, and paint; applications own tab and interaction state. |
...
| `ActionModal` | checked Ziran | `modal_widget.zi` composes ModalFrame, wrapped Text, and action Buttons; prompt text editing remains to migrate. |
...
| `WidgetKindTabBar` | `TabBar` | checked Ziran composition in `tab_bar_widget.zi` |
...
| `WidgetKindModal` | `ModalFrame`, `ActionModal` | checked Ziran frame and message/action dialog; prompt text editing remains |
