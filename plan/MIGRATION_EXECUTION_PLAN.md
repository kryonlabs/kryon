...
## Round 37 — the native-window host moves

`include/ui_window.h` is deleted: `runtime/window_props.kry` declares
`NativeWindow` as a one-line opaque `#type` (`struct NativeWindow NativeWindow;`)
— an opaque forward declaration is just C text, so even that "undeclarable"
case needed no emitter feature — plus the eleven window lifecycle/input calls
as externs. The window test binary builds, refuses an inherited display by
construction, and passes with a scrubbed environment
(`env -u DISPLAY -u WAYLAND_DISPLAY`: "window policy Kry checks passed").

Session total: thirty-eight handwritten UI headers removed. Gates: parity,
`fast-test`, all syntax tests, `go-runtime-test`, examples, boundary, the
window test, refreshed snapshots. Inbe builds green at `85d0cc187`; pointer
committed (`c36d80a`).

Remaining: `theme.h`/`theme_meta.h` (die with B-001 — deleting them now would
entrench the legacy catalog as tracked ABI), `ui_dpi.h` (perf-critical inline
getters, kept by design), `ui_icon_types.h` (generated by the icon script —
already generated output, not handwritten policy), and B-001 KSS/theme
downstream on Uku/Krait/Rill/Inbe. The handwriting-removal program has reached
