...
pressed, disabled, checked, selected, invalid, expanded, readonly, required,
and open when the browser has a matching concept. The web
...
`:nth-child(...)`/`:nth-last-child(...)`, plus simple `:not(...)`, `:is(...)`,
and `:where(...)`,
