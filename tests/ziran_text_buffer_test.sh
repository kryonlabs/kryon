...
source=$repo/tests/ziran_text_buffer_test.zi

"$ziran" ir --root "$repo/tests" --module-path "$repo/src/ui" \
    -o "$work/ir" "$source"
"$ziran" bundle --root "$repo/tests" --module-path "$repo/src/ui" \
    --entry ziran_text_buffer_test:main -o "$work/source.zib" "$source"
"$ziran" bundle --root "$work/ir" --module-path "$work/ir" \
    --entry ziran_text_buffer_test:main -o "$work/saved.zib" \
    "$work/ir/ziran_text_buffer_test.zir"
...
mv "$work/go/ziran_text_buffer_test.go" "$work/go/text_buffer_case.go"
cat > "$work/go/main.go" <<'GO'
package main
func main() {
    if ZiranTextBufferTest_Main() != 0 { panic("text buffer phase failed") }
}
GO
