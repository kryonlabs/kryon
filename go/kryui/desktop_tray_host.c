 ...
 * tray backend), so the app compiles the real thing here: the GTK
 * status-icon backend with lazy GTK (KRYON_TRAY_GTK_DL) — no link-time GTK
 * dependency, libgtk-3 is dlopen'd only when the tray starts. Desktop
 * notifications live in src/notification (GDBus, part of libkryon.a) and
 * need no host. Same setup inbe uses.
 */
