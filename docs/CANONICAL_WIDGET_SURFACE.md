...
| `src/ui/text_input_props.zi` | Portable TextField and TextArea props, raw input and supplied paste text, caller owned state, edit results, and clipboard write requests | checked Ziran |
...
| `TextArea` | `Collections` | Text area | `src/ui/text_input.zi`, `src/ui/text_input_props.zi`, `src/ui/text_area_widget.zi`, `src/ui/syntax.zi` | checked Ziran | Caller owned text and state, visual wrapping, pointer selection, vertical/page navigation, scrolling, edit and clipboard intents, KSS, Ziran/C/Make syntax coloring, composition underlines, and clipped paint are checked. IME event handling, platform clipboard IO, and native host integration remain. |
...
| `TextField` | checked Ziran | Caller owned text; retained focus, cursor, selection, scroll, KSS paint, UTF-8 byte edit intents, and copy/cut/paste decisions. IME, platform clipboard IO, and native host integration remain. |
| `TextArea` | checked Ziran | Caller owned text; wrapped rows, pointer selection, page navigation, scroll, KSS and syntax paint, composition underlines, multiline edit intents, and copy/cut/paste decisions. IME event handling, platform clipboard IO, and native host integration remain. |
...
| `TextField` | checked Ziran | `text_field_widget.zi` owns retained input, selection, scroll, KSS, paint, byte edit intents, and copy/cut/paste decisions; applications own text and focus state. IME, platform clipboard IO, and native host integration remain. |
| `TextArea` | checked Ziran | `text_area_widget.zi` and `syntax.zi` own visual rows, retained input, selection, scroll, KSS and syntax paint, composition underlines, multiline edit intents, and copy/cut/paste decisions; applications own text and focus state. IME event handling, platform clipboard IO, and native host integration remain. |
