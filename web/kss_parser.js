...
export function KssParser_KssDefaultEnvironment($rt, $state = moduleState, $host = moduleHost) {
  ...
  let env = {theme: 0, contrast: 0, density: 0, pointer: 0, platform: 0, variant: {bytes: Array.from({length: 64}, (_, index) => 0), length: 0}};
...
export function KssParser_KssBegin($rt, $state = moduleState, $host = moduleHost, source, path, env) {
  ...
  p.variant_count = kryon.copyValue(0);
...
export function KssParser_KssReadVariantLabel($rt, $state = moduleState, $host = moduleHost, c) {
  $state = $state || moduleState;
  /* pass-by-reference: KssCursor is same-module */
  $rt = $rt || kryon.createRuntime();
  let result = {parser: {source: "", pos: 0, line: 0, column: 0, file: 0}, name: {bytes: Array.from({length: 64}, (_, index) => 0), length: 0}, ok: false};
  c = KssParser_KssSkipSpace($rt, $state, $host, c);
  if (KssParser_KssAtEnd($rt, $state, $host, c) ||kryon.index( c.source, c.pos) != 34) {
    result.parser = kryon.copyValue(c);
    return result;
  }
  c = KssParser_KssAdvance($rt, $state, $host, c, 1);
...
export function KssParser_KssSetVariant($rt, $state = moduleState, $host = moduleHost, p, name) {
  $state = $state || moduleState;
  /* pass-by-reference: KssParser is same-module */
  /* pass-by-reference: KssName is same-module */
  $rt = $rt || kryon.createRuntime();
  p.env.variant = kryon.copyValue(name);
  return p;
  return kryon.snapshot($rt);
}
...
export function KssParser_KssVariantActive($rt, $state = moduleState, $host = moduleHost, p, name) {
  ...
  return KssParser_KssNameEqualsName($rt, $state, $host, p.env.variant, name);
}

export function KssParser_KssParseVariantBlock($rt, $state = moduleState, $host = moduleHost, p, name) {
  $state = $state || moduleState;
  /* pass-by-reference: KssParser is same-module */
  /* pass-by-reference: KssName is same-module */
  $rt = $rt || kryon.createRuntime();
  if (p.variant_count >= 8) {
    return KssParser_KssFail($rt, $state, $host, p, "style variant capacity exceeded");
  }
  p.variants[p.variant_count].name = kryon.copyValue(name);
  p.variants[p.variant_count].label = kryon.copyValue(p.variant_label);
  p.variant_count = kryon.copyValue(p.variant_count + 1);
  let open = KssParser_KssExpect($rt, $state, $host, p.cursor, 123);
  if (!open.ok) {
    p.cursor = kryon.copyValue(open.parser);
    return KssParser_KssFail($rt, $state, $host, p, "expected '{' after @variant name");
  }
  p.cursor = kryon.copyValue(open.parser);
  if (!KssParser_KssVariantActive($rt, $state, $host, p, name)) {
    return KssParser_KssSkipBlockRemainder($rt, $state, $host, p);
  }
  p.in_variant = kryon.copyValue(true);
  return KssParser_KssContinueVariantBlock($rt, $state, $host, p);
  return kryon.snapshot($rt);
}

export function KssParser_KssContinueVariantBlock($rt, $state = moduleState, $host = moduleHost, p) {
  $state = $state || moduleState;
  /* pass-by-reference: KssParser is same-module */
  $rt = $rt || kryon.createRuntime();
  let c = kryon.copyValue(p.cursor);
  ...
  while (running) {
    ...
    p.cursor = kryon.copyValue(c);
    if (KssParser_KssAtEnd($rt, $state, $host, c)) {
      return KssParser_KssFail($rt, $state, $host, p, "unterminated variant block");
    }
    if (kryon.index(c.source, c.pos) == 125) {
      c = KssParser_KssAdvance($rt, $state, $host, c, 1);
      p.cursor = kryon.copyValue(c);
      p.in_variant = kryon.copyValue(false);
      running = kryon.copyValue(false);
      continue;
    }
    let saved = kryon.copyValue(p);
    let entry = KssParser_KssTryOverlayEntry($rt, $state, $host, p, Math.trunc(Number(KssOriginVariant)));
    if (entry.parser.status == Math.trunc(Number(KssStatusError))) {
      return entry.parser;
    }
    if (entry.matched) {
...
export function KssParser_KssParseDirective($rt, $state = moduleState, $host = moduleHost, p) {
  ...
  if (KssParser_KssNameEquals($rt, $state, $host, keyword.name, "variant")) {
    let name = KssParser_KssReadName($rt, $state, $host, keyword.parser);
    if (!name.ok) {
      p.cursor = kryon.copyValue(name.parser);
      return KssParser_KssFail($rt, $state, $host, p, "expected variant name");
    }
    let label = KssParser_KssReadVariantLabel($rt, $state, $host, name.parser);
    if (!label.ok) {
      p.cursor = kryon.copyValue(label.parser);
      return KssParser_KssFail($rt, $state, $host, p, "expected variant label string");
    }
    p.variant_label = kryon.copyValue(label.name);
    p.cursor = kryon.copyValue(label.parser);
    return KssParser_KssParseVariantBlock($rt, $state, $host, p, name.name);
  }
...
export function KssParser_KssStep($rt, $state = moduleState, $host = moduleHost, p) {
  ...
  if (p.in_variant) {
    p.status = kryon.copyValue(Math.trunc(Number(KssStatusContinue)));
    p = KssParser_KssContinueVariantBlock($rt, $state, $host, p);
    if (p.status == Math.trunc(Number(KssStatusRule)) || p.status == Math.trunc(Number(KssStatusError))) {
      return p;
    }
    c = kryon.copyValue(p.cursor);
