...
`EndTreeCapture()` is an opt-in alternative to `EndTree()`. It commits a valid
...
paint overflow produces no capture or raster effects. Ordinary `EndTree()`
...
`PollPointer() -> PointerFrame` exactly once, and call `EndTreeCapture()` once.
...
tools/frame-replay.sh build/ziran/frame-capture-test/source.zib \
    frame_replay_demo tests/fixtures/frame_replay.trace \
