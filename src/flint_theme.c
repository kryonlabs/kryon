...
{
    Color highlight = flint_theme_get("lotus", "border_light");
    Color shadow = flint_theme_get("lotus", "border_shadow");
...
bool flint_theme_sync_to_apps(const char *src_scope)
{
    ...
    static const char *app_scopes[] = {
        "lotus", "clock", "display", "explorer", "inbe", "quest", "shell", "text", "uxn", NULL
    };

    for(int i = 0; app_scopes[i] != NULL; i++) {
        if(strcmp(app_scopes[i], src_scope) == 0)
            continue;

        for(int v = 0; v < source->count; v++) {
            const char *key = source->values[v].key;
            Color value = source->values[v].value;
            flint_theme_set_color(app_scopes[i], key, value);
        }
    }
    return true;
}
...
bool flint_theme_export_theme(const char *path)
{
    ...
    fprintf(file, "# Lotus theme export\n");
