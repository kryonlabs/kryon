...
void render_component_android(AndroidIRRenderer* ir_renderer,
    ...
    float x = parent_x + component->render_bounds.x;
    float y = parent_y + component->render_bounds.y;
    float width = component->render_bounds.width;
    float height = component->render_bounds.height;
    ...
    // Render box shadow first (if any)
    ...
    switch (component->type) {
        ...
        case IR_COMPONENT_INPUT: {
            ...
            // Render input text
            const char* text = component->input_data ?
                component->input_data->value : "";
        ...
        case IR_COMPONENT_CHECKBOX: {
            ...
            // If checked, render checkmark
            if (component->checkbox_data && component->checkbox_data->checked) {
                IRColor check_color = component->style ?
                    component->style->font.color :
                    (IRColor){0, 0, 0, 255};

                uint32_t color = ir_color_to_rgba(check_color, opacity);

                // Draw simple X as checkmark
                float padding = width * 0.2f;
                android_renderer_draw_rect(renderer,
                                          x + padding, y + height / 2 - 2,
                                          width - padding * 2, 4, color);
                android_renderer_draw_rect(renderer,
                                          x + width / 2 - 2, y + padding,
                                          4, height - padding * 2, color);
            }
            break;
        }
