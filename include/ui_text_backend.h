#ifndef KRYON_TEXT_BACKEND_H
#define KRYON_TEXT_BACKEND_H
...
int TextFontAdvance(Font font, int codepoint);

/* True if the font provides a glyph for `codepoint` (any glyph slot, not just
 * the loaded codepoint range). */
int TextFontHasGlyphValue(Font font, int codepoint);

/* Number of rasterized glyphs in the fonts atlas (font.glyphCount), >= 0.
 * Used by diagnostics; regular UI code should not need it. */
int TextFontGlyphCount(Font font);

/* Native backend text path. Returns false/0 on backends that only support
 * atlas-backed fonts. */
int TextFontHasNativeText(Font font);
int TextFontNativeTextWidth(Font font, const char *text, int byte_len);
int TextFontNativeTextHeight(Font font);
int TextFontDrawNativeText(Font font, const char *text, int byte_len, int x,
                         int y, int font_size, Color color);

#endif /* KRYON_TEXT_BACKEND_H */
