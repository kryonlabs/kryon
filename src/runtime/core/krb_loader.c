...
static bool ensure_script_function_capacity(KryonRuntime* runtime, size_t required_count) {
    ...
    if (!new_storage) {
        fprintf(stderr, "❌ Failed to allocate script function storage (requested %zu entries)\n", new_capacity);
        return false;
    }
...
static const char *get_element_type_name(uint16_t hex) {
    ...
    if (hex >= 0x1000) {
        ...
        snprint(custom_name, sizeof(custom_name), "CustomElement_0x%04X", hex);
        return custom_name;
    }
...
static const char *get_property_name(uint16_t hex) {
    ...
    if (hex >= 0x9000) {
        ...
        snprint(custom_prop_name, sizeof(custom_prop_name), "customProp_0x%04X", hex);
...
bool kryon_runtime_load_krb_data(KryonRuntime *runtime, const uint8_t *data, size_t size) {
    ...
    if (!read_uint32_safe(data, &offset, size, &magic)) {
        fprintf(stderr, "DEBUG: Failed to read magic number\n");
        return false;
    }
    if (magic != 0x4B52594E) { // "KRYN"
        fprintf(stderr, "DEBUG: Invalid magic number: 0x%08X (expected 0x4B52594E)\n", magic);
        return false;
    }
    ...
    if (!read_uint16_safe(data, &offset, size, &version_major) ||
        ...
        fprintf(stderr, "DEBUG: Failed to read version\n");
        return false;
    }
    
    uint16_t flags;
    if (!read_uint16_safe(data, &offset, size, &flags)) {
        fprintf(stderr, "DEBUG: Failed to read flags\n");
        return false;
    }
    
    fprintf(stderr, "DEBUG: KRB file v%u.%u.%u, flags: 0x%04X\n", version_major, version_minor, version_patch, flags);
    ...
    if (!read_uint32_safe(data, &offset, size, &style_count) ||
        ...
        fprintf(stderr, "DEBUG: Failed to read section counts\n");
        return false;
    }
    ...
    if (!read_uint32_safe(data, &offset, size, &string_table_size) ||
        ...
        fprintf(stderr, "DEBUG: Failed to read size/checksum fields\n");
        return false;
    }
    ...
    if (!read_uint8_safe(data, &offset, size, &compression) ||
        ...
        fprintf(stderr, "DEBUG: Failed to read compression fields\n");
        return false;
    }
    ...
    if (!read_uint32_safe(data, &offset, size, &style_offset) ||
        ...
        fprintf(stderr, "DEBUG: Failed to read section offsets\n");
        return false;
    }
    ...
    if (offset != 128) {
        fprintf(stderr, "DEBUG: Header size mismatch: %zu (expected 128)\n", offset);
        return false;
    }
    
    fprintf(stderr, "DEBUG: Found %u styles, %u themes, %u widget defs, %u elements, %u properties in KRB file\n", 
           style_count, theme_count, element_def_count, element_count, property_count);
    fprintf(stderr, "DEBUG: Section offsets - Style: %u, Script: %u, String table: %u\n", 
           style_offset, script_offset, string_table_offset);
    
    // Load string table from its stored offset
    fprintf(stderr, "DEBUG: String table size from header: %u bytes (0x%08X), offset: %u\n", 
    ...
    if (string_table_size > 0 && string_table_offset > 0) {
        if (string_table_size > 1000000) { // If > 1MB, something's wrong
            fprintf(stderr, "DEBUG: String table size seems too large, possible endianness issue\n");
            return false;
        }
        ...
        if (!read_uint32_safe(data, &offset, size, &string_count)) {
            fprintf(stderr, "DEBUG: Failed to read string count\n");
            return false;
        }
        
        fprintf(stderr, "DEBUG: Loading %u strings from string table\n", string_count);
        
        // Allocate string table (handle 0 count case)
        if (string_count > 0) {
            string_table = kryon_alloc(string_count * sizeof(char*));
            if (!string_table) {
                fprintf(stderr, "DEBUG: Failed to allocate string table\n");
                return false;
            }
        ...
        for (uint32_t i = 0; i < string_count; i++) {
            ...
            if (!read_uint16_safe(data, &offset, size, &str_len)) {
                fprintf(stderr, "DEBUG: Failed to read string length\n");
                goto cleanup;
            }
            
            if (offset + str_len > size) {
                fprintf(stderr, "DEBUG: String data exceeds buffer size\n");
                goto cleanup;
            }
            
            string_table[i] = kryon_alloc(str_len + 1);
            if (!string_table[i]) {
                fprintf(stderr, "DEBUG: Failed to allocate string %u\n", i);
                goto cleanup;
            }
            ...
            fprintf(stderr, "DEBUG: String[%u]: '%s'\n", i, string_table[i]);
        }
        
        // Verify we consumed the expected bytes
        if (offset - string_table_start != string_table_size) {
            fprintf(stderr, "DEBUG: String table size mismatch: expected %u, got %zu\n", 
    ...
    if (element_count > 0 && element_inst_offset > 0) {
        fprintf(stderr, "DEBUG: Jumping to widget instance section at offset %u\n", element_inst_offset);
    ...
    if (element_count > 0) {
        ...
        if (!runtime->root) {
            fprintf(stderr, "DEBUG: Failed to load root element\n");
            return false;
        }
        fprintf(stderr, "DEBUG: Successfully loaded root element\n");
        if (runtime->root && runtime->root->type_name) {
            fprintf(stderr, "DEBUG: Root element type: %s\n", runtime->root->type_name);
        }
        fprintf(stderr, "DEBUG: Element loading complete, about to load variables\n");
    } else {
        fprintf(stderr, "DEBUG: No elements found in KRB file\n");
    }
                            ...
                            if (value_type == KRYON_VALUE_STRING) {
                                ...
                                if (read_uint32_safe(data, &vars_offset, size, &value_ref)) {
                                    if (value_ref < string_count && string_table[value_ref]) {
                                        ...
                                        fprintf(stderr, "DEBUG: Loaded STRING variable '%s' = '%s'\n", var_name, string_value);
                                    }
                                }
                                ...
                                if (read_uint32_safe(data, &vars_offset, size, &raw_value)) {
                                    ...
                                    char int_buffer[32];
                                    snprint(int_buffer, sizeof(int_buffer), "%d", int_value);
                                    runtime->variable_names[runtime->variable_count] = kryon_strdup(var_name);
                                    runtime->variable_values[runtime->variable_count] = kryon_strdup(int_buffer);
                                    runtime->variable_count++;
                                    fprintf(stderr, "DEBUG: Loaded INTEGER variable '%s' = %d\n", var_name, int_value);
                                }
                                ...
                                if (read_uint8_safe(data, &vars_offset, size, &bool_value)) {
                                    ...
                                    runtime->variable_count++;
                                    fprintf(stderr, "DEBUG: Loaded BOOLEAN variable '%s' = %s\n", var_name, bool_value ? "true" : "false");
                                }
                            } else {
                                fprintf(stderr, "DEBUG: Skipped unknown variable type %d for '%s'\n", value_type, var_name);
                            }
                        }
                        
                        fprintf(stderr, "✅ PHASE 1: Loaded %zu variables before function loading\n", runtime->variable_count);
                        fprintf(stderr, "🔄 Reprocessing property bindings with loaded variables\n");
    ...
    // ======================================================================
    if (script_offset > 0) {
        fprintf(stderr, "🔄 PHASE 2: Loading Functions section AFTER variables at offset %u\n", script_offset);
        ...
        fprintf(stderr, "DEBUG: Searching for FUNC magic starting from offset %zu\n", search_offset);
        
        for (size_t i = search_offset; i <= size - 4; i++) {
            uint32_t magic = (data[i] << 24) | (data[i+1] << 16) | (data[i+2] << 8) | data[i+3];
            if (magic == 0x46554E43) { // "FUNC"
                fprintf(stderr, "DEBUG: Found FUNC magic at offset %zu\n", i);
                func_offset = i;
                found_func = true;
                break;
            }
        ...
        if (!found_func) {
            fprintf(stderr, "DEBUG: No FUNC section found, skipping function loading\n");
        } else {
            ...
            if (!read_uint32_safe(data, &offset, size, &function_count)) {
                fprintf(stderr, "DEBUG: Failed to read function count\n");
                return false;
            }
            
            fprintf(stderr, "DEBUG: Found %u functions in FUNC section\n", function_count);
            
            if (function_count > 100) {
                fprintf(stderr, "DEBUG: Function count seems too high (%u), skipping function loading\n", function_count);
            } else {
                // Load each function
                for (uint32_t i = 0; i < function_count; i++) {
                    if (!load_function_from_binary(runtime, data, &offset, size, string_table, string_count)) {
                        fprintf(stderr, "DEBUG: Failed to load function %u\n", i);
    ...
    if (offset < size) {
        ...
        if (read_uint32_safe(data, &comp_offset, size, &comp_magic) && comp_magic == 0x434F4D50) {
            fprintf(stderr, "DEBUG: Loading components section at offset %zu\n", offset);
            ...
            while (comp_offset < size) {
                ...
                fprintf(stderr, "🔄 Attempting to load component at offset %zu\n", comp_offset);
                if (!load_components_section(runtime, data, size, &comp_offset, string_table, string_count)) {
                    ...
                    fprintf(stderr, "⚠️  Component loading failed, stopping component section processing\n");
                    break;
                }
                
                // Make sure we made progress
                if (comp_offset <= section_start) {
                    fprintf(stderr, "DEBUG: Component loading made no progress, stopping\n");
                    break;
                }
            }
            ...
            for (size_t i = 0; i < runtime->component_count; i++) {
                fprintf(stderr, "  [%zu] %s (%zu params, %zu state vars)\n", i, 
...
static KryonElement *load_element_simple(KryonRuntime *runtime,
                                         ...
                                         uint32_t string_count,
                                         uint32_t *next_id) {
    fprintf(stderr, "🔄 Loading simplified element at offset %zu\n", *offset);

    // Read element type (uint16)
    uint16_t element_type;
    if (!read_uint16_safe(data, offset, size, &element_type)) {
        fprintf(stderr, "❌ Failed to read element type\n");
        return NULL;
    }

    fprintf(stderr, "🔍 Element type: 0x%04X\n", element_type);
    ...
    if (!read_uint16_safe(data, offset, size, &prop_count)) {
        fprintf(stderr, "❌ Failed to read property count\n");
        kryon_free(element);
        return NULL;
    }

    fprintf(stderr, "🔍 Property count: %u\n", prop_count);
    ...
    if (!load_element_properties(element, data, offset, size, prop_count, runtime)) {
        fprintf(stderr, "❌ Failed to load element properties\n");
    ...
    if (!read_uint16_safe(data, offset, size, &child_count)) {
        fprintf(stderr, "❌ Failed to read child count\n");
        kryon_free(element->properties);
        kryon_free((void*)element->type_name);
        kryon_free(element);
        return NULL;
    ...
    if (child_count > 0) {
        ...
        for (size_t i = 0; i < child_count; i++) {
            ...
            if (!element->children[i]) {
                fprintf(stderr, "❌ Failed to load child %zu\n", i);
                // Cleanup
    ...
    fprintf(stderr, "✅ Loaded simplified element: %s (id=%u, %u props, %u children)\n",
...
static KryonElement *load_element_from_binary(KryonRuntime *runtime,
    ...
    fprintf(stderr, "🔄 Loading element at offset %zu\n", *offset);
    
    // Validate and read element header using schema
    if (!krb_validate_element_header(data, size, offset, &header)) {
        fprintf(stderr, "❌ Failed to validate element header at offset %zu\n", *offset);
        return NULL;
    }
    
    fprintf(stderr, "🏗️  Loaded element header: id=%u type=0x%x props=%u children=%u events=%u\n", 
    ...
    if (type_name) {
        ...
        fprintf(stderr, "❌ ERROR: Unknown element type ID: %u\n", header.element_type);
    ...
    if (header.property_count > 0) {
        fprintf(stderr, "DEBUG: About to load %u properties for element %s\n", header.property_count, element->type_name);
    ...
    fprintf(stderr, "🔍 DEBUG: Element has element_type=0x%04X, checking if >= 0x2000 for component instance\n", 
           header.element_type);
    if (element->type_name) {
        fprintf(stderr, "🔍 DEBUG: Element type name: %s\n", element->type_name);
    } else {
        fprintf(stderr, "🔍 DEBUG: Element type name: (null)\n");
    }
                    ...
                    if (comp_inst) {
                        ...
                        for (size_t j = 0; j < comp_inst->definition->state_count; j++) {
                            ...
                            kryon_component_state_set_string(comp_inst->state_table,
                                                             ...
                                                             default_val);
                            fprintf(stderr, "    Initialized state '%s' to '%s'\n",
                                   comp_inst->definition->state_vars[j].name, default_val);
                        }
                        
                                                
                        element->component_instance = comp_inst;
                        fprintf(stderr, "✅ Successfully created component instance for '%s'\n", runtime->components[i]->name);
                    }
                    break;
                }
            }
    ...
    for (uint32_t i = 0; i < header.child_count; i++) {
        ...
        //         fprintf(stderr, "🔍 PRE-CHILD DEBUG: string_table[%zu] = %p -> '%s'\n", 
        ...
        if (!child) {
            fprintf(stderr, "DEBUG: Failed to load child %u\n", i+1);
            kryon_element_destroy(runtime, element);
            return NULL;
        }
        ...
        // fprintf(stderr, "DEBUG: Successfully loaded child %u: %s\n", i+1, child->type_name ? child->type_name : "(null)");
    }
...
static bool load_element_properties(KryonElement *element,
    ...
    for (uint32_t i = 0; i < property_count; i++) {
        ...
        if (!krb_validate_property_header(data, size, offset, &prop_header)) {
            fprintf(stderr, "❌ Failed to validate property header at offset %zu\n", *offset);
            return false;
        }
        
        // Skip debug output to avoid printf crashes
        // fprintf(stderr, "🔧 Loaded property header: id=0x%04x type=%u\n", (unsigned int)prop_header.property_id, (unsigned int)prop_header.value_type);
        ...
        if (prop_name) {
            ...
            fprintf(stderr, "❌ ERROR: Unknown property ID: 0x%04X\n", prop_header.property_id);
        ...
        // fprintf(stderr, "DEBUG: Loading property %s (id=0x%04X, type=%d)\n", 
        ...
        if (prop_header.property_id == kryon_get_property_hex("id")) { // "id" property
            ...
            // Read property value
            fprintf(stderr, "🔍 PROPERTY DEBUG: About to call load_property_value for property type=%d\n", property->type);
            fprintf(stderr, "🔍 PROPERTY DEBUG: About to check property pointer\n");
            if (!property) {
                fprintf(stderr, "❌ PROPERTY DEBUG: Property pointer is NULL!\n");
                return false;
            }
            fprintf(stderr, "🔍 PROPERTY DEBUG: Property pointer is valid\n");
            fprintf(stderr, "🔍 PROPERTY DEBUG: Property name exists: %s\n", property->name ? "yes" : "no");
            if (!load_property_value(property, data, offset, size, runtime, element)) {
                kryon_free(property->name);
                kryon_free(property);
                return false;
        ...
        fprintf(stderr, "🔍 ARRAY DEBUG: About to add property %u to element\n", i);
        fprintf(stderr, "🔍 ARRAY DEBUG: Current property_count: %zu\n", element->property_count);
        fprintf(stderr, "🔍 ARRAY DEBUG: Property array capacity: %u\n", property_count);
        
        if (element->property_count >= property_count) {
            fprintf(stderr, "❌ ARRAY DEBUG: Property array overflow!\n");
            return false;
        }
        
        element->properties[element->property_count++] = property;
        fprintf(stderr, "🔍 ARRAY DEBUG: Successfully added property, new count: %zu\n", element->property_count);
    }
        ...
        case KRYON_RUNTIME_PROP_STRING:
            {
                ...
                if (!property->value.string_value) {
                    fprintf(stderr, "DEBUG: Failed to allocate string memory\n");
                    return false;
                }
        ...
        case KRYON_RUNTIME_PROP_FLOAT:
            {
                ...
                if (!read_uint32_safe(data, offset, size, &float_bits)) {
                    fprintf(stderr, "DEBUG: Failed to read float value\n"); 
                    return false;
                }
                float float_value;
                ...
                if (float_value == -99999.0f && runtime) {
                    ...
                    if (property->name && strcmp(property->name, "posY") == 0) {
                        ...
                        if (var_value) {
                            ...
                            fprintf(stderr, "🔄 Resolved float reactive variable '%s' to %f (from root.height='%s')\n", 
                                   property->name ? property->name : "(null)", resolved_value, var_value);
                        } else {
                            property->value.float_value = 0.0; // Default fallback
                            fprintf(stderr, "⚠️  Failed to resolve reactive variable for '%s', using 0.0\n", property->name);
                        }
                        ...
                        if (var_value) {
                            float resolved_value = (float)atof(var_value);
                            property->value.float_value = (double)resolved_value;
                            fprintf(stderr, "🔄 Resolved float reactive variable '%s' to %f (from root.width='%s')\n", 
                                   property->name ? property->name : "(null)", resolved_value, var_value);
                        } else {
                            property->value.float_value = 0.0; // Default fallback
                            fprintf(stderr, "⚠️  Failed to resolve reactive variable for '%s', using 0.0\n", property->name);
                        ...
                        fprintf(stderr, "🔗 Float property '%s' marked as reactive (sentinel detected)\n", property->name);
                    }
                } else {
                    property->value.float_value = (double)float_value;
                    fprintf(stderr, "DEBUG: Read float property '%s' = %f\n", property->name ? property->name : "(null)", float_value);
                }
            }
            break;
        ...
        case KRYON_RUNTIME_PROP_COLOR:
            {
                ...
                if (!read_uint32_safe(data, offset, size, &color_value)) {
                    fprintf(stderr, "DEBUG: Failed to read color value\n");
                    return false;
                }
                property->value.color_value = color_value;
                fprintf(stderr, "DEBUG: Read color property '%s' = 0x%08X\n", property->name ? property->name : "(null)", color_value);
            }
            break;
        ...
        case KRYON_RUNTIME_PROP_FUNCTION:
            {
                ...
                uint16_t string_length;
                if (!read_uint16_safe(data, offset, size, &string_length)) {
                    fprintf(stderr, "DEBUG: Failed to read function name length\n");
                    return false;
                }
                
                if (*offset + string_length > size) {
                    fprintf(stderr, "DEBUG: Function name data exceeds buffer size\n");
                    return false;
                }
                
                property->value.string_value = kryon_alloc(string_length + 1);
                if (!property->value.string_value) {
                    fprintf(stderr, "DEBUG: Failed to allocate function name string\n");
                    return false;
                }
                ...
                *offset += string_length;
                ...
                fprintf(stderr, "DEBUG: Read function property '%s' = '%s'\n", property->name ? property->name : "(null)", property->value.string_value);
            }
            break;
        ...
        case KRYON_RUNTIME_PROP_REFERENCE:
            {
                ...
                if (!read_uint8_safe(data, offset, size, &prop_type)) {
                    fprintf(stderr, "DEBUG: Failed to read REFERENCE property type prefix\n");
                    return false;
                }
                
                if (prop_type != KRYON_RUNTIME_PROP_REFERENCE) {
                    fprintf(stderr, "DEBUG: Expected REFERENCE type prefix, got %d\n", prop_type);
                    return false;
                }
                ...
                if (!read_uint32_safe(data, offset, size, &variable_name_ref)) {
                    fprintf(stderr, "DEBUG: Failed to read variable name reference\n");
                    return false;
                }
                
                // Safely access string table with comprehensive validation
                // fprintf(stderr, "🔍 REFERENCE DEBUG: Processing reference property\n");
                // fprintf(stderr, "🔍 REFERENCE DEBUG: variable_name_ref value is available\n");
                // fprintf(stderr, "🔍 REFERENCE DEBUG: runtime structure is valid\n");
                
                const char* variable_name = NULL;
                
                // Validate string table and reference bounds
                if (!runtime->string_table) {
                    // fprintf(stderr, "❌ REFERENCE DEBUG: String table is NULL!\n");
                    ...
                    // fprintf(stderr, "❌ REFERENCE DEBUG: Invalid reference %u >= %zu\n", 
                    ...
                    // fprintf(stderr, "🔍 REFERENCE DEBUG: string_table[%u] = %p\n", variable_name_ref, (void*)string_ptr);
                    
                    if (!string_ptr) {
                        // fprintf(stderr, "❌ REFERENCE DEBUG: String pointer is NULL at index %u\n", variable_name_ref);
                        ...
                        // fprintf(stderr, "✅ REFERENCE DEBUG: Valid string '%s' (ref=%u)\n", 
                        ...
                        fprintf(stderr, "🔗 Property '%s' bound to variable '%s'\n", 
        ...
        case KRYON_RUNTIME_PROP_TEMPLATE:
            {
                ...
                uint8_t prop_type;
                if (!read_uint8_safe(data, offset, size, &prop_type)) {
                    fprintf(stderr, "DEBUG: Failed to read TEMPLATE property type prefix\n");
                    return false;
                }
                
                if (prop_type != KRYON_RUNTIME_PROP_TEMPLATE) {
                    fprintf(stderr, "DEBUG: Expected TEMPLATE type prefix, got %d\n", prop_type);
                    return false;
                }
                ...
                if (!read_uint16_safe(data, offset, size, &segment_count)) {
                    fprintf(stderr, "DEBUG: Failed to read template segment count\n");
                    return false;
                }
                
                fprintf(stderr, "📝 Loading template with %u segments\n", segment_count);
                ...
                if (!property->value.template_value.segments) {
                    fprintf(stderr, "DEBUG: Failed to allocate template segments\n");
                    return false;
                }
                ...
                for (uint16_t i = 0; i < segment_count; i++) {
                    ...
                    if (!read_uint8_safe(data, offset, size, &segment_type)) {
                        fprintf(stderr, "DEBUG: Failed to read segment %u type\n", i);
                        return false;
                    }
                    ...
                    if (segment_type == KRYON_TEMPLATE_SEGMENT_LITERAL) {
                        ...
                        if (!read_uint16_safe(data, offset, size, &text_len)) {
                            fprintf(stderr, "DEBUG: Failed to read literal text length for segment %u\n", i);
                            return false;
                        }
                        
                        if (*offset + text_len > size) {
                            fprintf(stderr, "DEBUG: Literal text data exceeds buffer size for segment %u\n", i);
                            return false;
                        }
                        
                        segment->data.literal_text = kryon_alloc(text_len + 1);
                        if (!segment->data.literal_text) {
                            fprintf(stderr, "DEBUG: Failed to allocate literal text for segment %u\n", i);
                            return false;
                        }
                        ...
                        *offset += text_len;
                        ...
                        fprintf(stderr, "  📝 Loaded literal segment %u: '%s'\n", i, segment->data.literal_text);
                        ...
                        if (!read_uint32_safe(data, offset, size, &var_name_ref)) {
                            fprintf(stderr, "DEBUG: Failed to read variable name reference for segment %u\n", i);
                            return false;
                        }
                        ...
                        if (var_name_ref < runtime->string_table_count) {
                            ...
                            fprintf(stderr, "  🔗 Loaded variable segment %u: '%s' (string_ref=%u)\n", 
                                   i, segment->data.variable_name, var_name_ref);
                        } else {
                            fprintf(stderr, "DEBUG: Invalid variable name reference %u for segment %u\n", var_name_ref, i);
                            segment->data.variable_name = kryon_strdup("");
                        }
                        
                    } else {
                        fprintf(stderr, "DEBUG: Unknown template segment type %u for segment %u\n", segment_type, i);
                        return false;
                    }
                }
                
                fprintf(stderr, "✅ Template property loaded with %u segments\n", segment_count);
            }
            break;

        default:
            fprintf(stderr, "DEBUG: Unknown property type %d for property '%s' - skipping\n", property->type, property->name);
...
bool kryon_element_set_property(KryonElement *element, uint16_t property_id, const void *value) {
    ...
    if (prop_name) {
        ...
        if (property_id == 0x0606 || property_id == 0x0607) {
            fprintf(stderr, "🔍 KRB PROPERTY LOAD: hex=0x%04X → name='%s'\n", property_id, prop_name);
        }
    } else {
        fprintf(stderr, "❌ ERROR: Unknown property ID: 0x%04X\n", property_id);
...
static void register_event_directive_handlers(KryonRuntime *runtime, KryonElement *element) {
    ...
    fprintf(stderr, "DEBUG: Registering event handlers for event directive\n");
    ...
    for (size_t i = 0; i < element->property_count; i++) {
        ...
        fprintf(stderr, "DEBUG: Found event handler property: %s -> %s\n", 
        ...
        if (strncmp(prop->name, "keyboard:", 9) == 0) {
            ...
            fprintf(stderr, "DEBUG: Would register keyboard handler for '%s' -> '%s'\n", 
            ...
            fprintf(stderr, "DEBUG: Would register mouse handler for '%s' -> '%s'\n", 
...
static bool skip_styles_section(const uint8_t *data, size_t *offset, size_t size, uint32_t style_count) {
    ...
    fprintf(stderr, "DEBUG: Successfully skipped %u styles\n", style_count);
    return true;
}
...
static bool load_function_from_binary(KryonRuntime *runtime, const uint8_t *data, size_t *offset, size_t size, char** string_table, uint32_t string_count) {
    ...
    if (!read_uint32_safe(data, offset, size, &func_magic)) {
        fprintf(stderr, "DEBUG: Failed to read function magic\n");
        return false;
    }
    
    if (func_magic != KRB_MAGIC_FUNC) {
        fprintf(stderr, "DEBUG: Invalid function magic: 0x%08X (expected 0x%08X)\n", func_magic, KRB_MAGIC_FUNC);
        return false;
    }
    
    uint32_t lang_ref;
    if (!read_uint32_safe(data, offset, size, &lang_ref)) {
        fprintf(stderr, "DEBUG: Failed to read language reference\n");
        return false;
    }

    uint32_t name_ref;
    if (!read_uint32_safe(data, offset, size, &name_ref)) {
        fprintf(stderr, "DEBUG: Failed to read name reference\n");
        return false;
    }
    ...
    if (!read_uint16_safe(data, offset, size, &param_count)) {
        fprintf(stderr, "DEBUG: Failed to read parameter count\n");
        return false;
    }
    
    fprintf(stderr, "DEBUG: Function header - magic=0x%08X, lang_ref=%u, name_ref=%u, param_count=%u (offset now %zu)\n", 
    ...
    if (!ensure_script_function_capacity(runtime, function_index + 1)) {
        fprintf(stderr, "❌ Failed to reserve slot for function index %zu\n", function_index);
        return false;
    }
    ...
    if (param_count > 0) {
        ...
        if (!function->parameters) {
            fprintf(stderr, "❌ Failed to allocate parameter list for function '%s' (%u params)\n", function->name ? function->name : "<null>", param_count);
    ...
    for (uint16_t i = 0; i < param_count; i++) {
        ...
        if (!read_uint32_safe(data, offset, size, &param_ref)) {
            fprintf(stderr, "DEBUG: Failed to read parameter reference %u\n", i);
            reset_script_function(function);
            return false;
        }
    ...
    uint16_t flags;
    if (!read_uint16_safe(data, offset, size, &flags)) {
        fprintf(stderr, "DEBUG: Failed to read function flags\n");
        reset_script_function(function);
        return false;
    }
    fprintf(stderr, "DEBUG: Read flags=0x%04X at offset %zu\n", flags, *offset - 2);

    // Read function code reference (string table index - contains script source)
    uint32_t code_ref;
    fprintf(stderr, "DEBUG: About to read code_ref at offset %zu\n", *offset);
    if (!read_uint32_safe(data, offset, size, &code_ref)) {
        fprintf(stderr, "DEBUG: Failed to read code reference\n");
        reset_script_function(function);
        return false;
    }
    fprintf(stderr, "DEBUG: Read code_ref=%u at offset %zu\n", code_ref, *offset - 4);
    ...
    if (!function->name || !function->language || !function->code) {
        fprintf(stderr, "❌ Failed to duplicate strings for function (name_ref=%u, code_ref=%u)\n", name_ref, code_ref);
        reset_script_function(function);
        return false;
    }
...
static bool load_components_section(KryonRuntime* runtime, const uint8_t* data, size_t size, size_t* offset, char** string_table, uint32_t string_count) {
    fprintf(stderr, "📖 Reading component at offset %zu\n", *offset);
    
    uint32_t comp_magic;
    if (!read_uint32_safe(data, offset, size, &comp_magic)) {
        fprintf(stderr, "❌ Failed to read component magic\n");
        return false;
    }
    
    fprintf(stderr, "🔍 Component magic: 0x%08X (expected 0x434F4D50)\n", comp_magic);
    
    if (comp_magic != 0x434F4D50) { // "COMP"
        fprintf(stderr, "❌ Invalid component magic: 0x%08X\n", comp_magic);
        return false;
    }
    ...
    uint32_t name_ref;
    if (!read_uint32_safe(data, offset, size, &name_ref)) {
        fprintf(stderr, "❌ Failed to read component name reference\n");
        return false;
    }
    
    fprintf(stderr, "🔍 Component name ref: %u (string_count=%u)\n", name_ref, string_count);
    ...
    if (string_index >= string_count || !string_table[string_index]) {
        fprintf(stderr, "❌ Invalid component name reference: ref=%u, string_index=%u, string_count=%u, string_exists=%s\n", 
    ...
    uint16_t param_count;
    if (!read_uint16_safe(data, offset, size, &param_count)) {
        fprintf(stderr, "❌ Failed to read parameter count\n");
        kryon_free(comp_def->name);
        kryon_free(comp_def);
        return false;
    }
    
    fprintf(stderr, "🔍 Parameter count: %u\n", param_count);
    
    comp_def->parameter_count = param_count;
    
    // Allocate parameter arrays
    fprintf(stderr, "🔍 Allocating arrays for %u parameters\n", param_count);
    if (param_count > 0) {
        ...
        if (!comp_def->parameters || !comp_def->param_defaults) {
            fprintf(stderr, "❌ Failed to allocate parameter arrays\n");
            kryon_free(comp_def->name);
        ...
        for (size_t i = 0; i < param_count; i++) {
            fprintf(stderr, "🔍 Reading parameter %zu/%u\n", i+1, param_count);
            
            // Read parameter name reference
            uint32_t param_ref;
            if (!read_uint32_safe(data, offset, size, &param_ref)) {
                fprintf(stderr, "❌ Failed to read param_ref for parameter %zu\n", i);
                // Cleanup on error
                for (size_t j = 0; j < i; j++) {
            ...
            if (!read_uint8_safe(data, offset, size, &has_default)) {
                fprintf(stderr, "❌ Failed to read has_default for parameter %zu\n", i);
                // Cleanup on error
                for (size_t j = 0; j < i; j++) {
                    kryon_free(comp_def->parameters[j]);
                    kryon_free(comp_def->param_defaults[j]);
            ...
            if (has_default) {
                ...
                if (!read_uint32_safe(data, offset, size, &default_ref)) {
                    fprintf(stderr, "❌ Failed to read default_ref for parameter %zu\n", i);
                    // Cleanup on error
                    for (size_t j = 0; j <= i; j++) {
                        kryon_free(comp_def->parameters[j]);
                        kryon_free(comp_def->param_defaults[j]);
                ...
                fprintf(stderr, "🔍 Parameter default: '%s'\n", comp_def->param_defaults[i] ? comp_def->param_defaults[i] : "(null)");
            } else {
                comp_def->param_defaults[i] = NULL;
                fprintf(stderr, "🔍 Parameter default: (none)\n");
            }
        }
    }
    
    // Read state variable count
    fprintf(stderr, "🔍 About to read state_count at offset %zu\n", *offset);
    uint16_t state_count;
    if (!read_uint16_safe(data, offset, size, &state_count)) {
        fprintf(stderr, "❌ Failed to read state_count\n");
        // Cleanup
        for (size_t i = 0; i < param_count; i++) {
    ...
    fprintf(stderr, "✅ Successfully read state_count: %u\n", state_count);

    // Allocate state variables array
    if (state_count > 0) {
        fprintf(stderr, "🔍 Allocating %u state variables\n", state_count);
        ...
        for (size_t i = 0; i < state_count; i++) {
            ...
            fprintf(stderr, "🔍 Reading state variable %zu at offset %zu\n", i, *offset);
            uint8_t var_flags;
            if (!read_uint32_safe(data, offset, size, &var_name_ref)) {
                fprintf(stderr, "❌ Failed to read name_ref for state variable %zu\n", i);
                // ... cleanup ...
            }
            if (!read_uint8_safe(data, offset, size, &var_type)) {
                fprintf(stderr, "❌ Failed to read var_type for state variable %zu\n", i);
                // ... cleanup ...
            }
            if (!read_uint8_safe(data, offset, size, &var_flags)) {
                fprintf(stderr, "❌ Failed to read var_flags for state variable %zu\n", i);
                // Cleanup
                for (size_t j = 0; j < i; j++) {
            ...
            fprintf(stderr, "🔍 State var %zu: name_ref=%u, type=%u\n", i, var_name_ref, var_type);

            if (var_name_ref >= string_count || !string_table[var_name_ref]) {
                fprintf(stderr, "❌ Invalid string ref: var_name_ref=%u >= string_count=%u\n", var_name_ref, string_count);
                // Cleanup on error
                for (size_t j = 0; j < i; j++) {
                    kryon_free(comp_def->state_vars[j].name);
                    kryon_free(comp_def->state_vars[j].default_value);
            ...
            fprintf(stderr, "🔍 Read value_type_tag=%u for state var %zu\n", value_type_tag, i);

            // Read default value based on value type tag
            if (value_type_tag == KRYON_VALUE_STRING) {
                uint32_t value_ref;
                fprintf(stderr, "🔍 Reading string value_ref at offset %zu (size=%zu)\n", *offset, size);
                if (!read_uint32_safe(data, offset, size, &value_ref)) {
                    fprintf(stderr, "❌ Failed to read string value_ref (offset=%zu, size=%zu)\n", *offset, size);
                    comp_def->state_vars[i].default_value = NULL;
                } else {
                    fprintf(stderr, "✅ Read string value_ref=%u\n", value_ref);
                ...
                if (!read_int64_safe(data, offset, size, &int_val)) {
                    ...
                    char int_str[32];
                    snprint(int_str, sizeof(int_str), "%lld", (long long)int_val);
    ...
    if (!read_uint16_safe(data, offset, size, &function_count)) {
        fprintf(stderr, "❌ Failed to read function count\n");
    ...
    fprintf(stderr, "🔍 Function count: %u\n", function_count);
    ...
    for (size_t i = 0; i < function_count; i++) {
        ...
        if (!read_uint32_safe(data, offset, size, &func_name_ref) ||
            ...
            fprintf(stderr, "❌ Failed to skip function %zu\n", i);
            break;
        }
        
        if (has_default) {
            if (!read_uint32_safe(data, offset, size, &func_code_ref)) {
                fprintf(stderr, "❌ Failed to skip function code %zu\n", i);
                break;
            }
        }
    }
    ...
    uint8_t has_body;
    fprintf(stderr, "🔍 Reading has_body at offset %zu\n", *offset);
    if (!read_uint8_safe(data, offset, size, &has_body)) {
        fprintf(stderr, "❌ Failed to read has_body flag\n");
        // Cleanup
        for (size_t i = 0; i < state_count; i++) {
            kryon_free(comp_def->state_vars[i].name);
            kryon_free(comp_def->state_vars[i].default_value);
    ...
    fprintf(stderr, "🔍 Component has body: %s\n", has_body ? "yes" : "no");
    
    if (has_body) {
        // Load component body element as ui_template
        fprintf(stderr, "🔄 Loading component body element as ui_template\n");
    ...
    fprintf(stderr, "✅ Loaded component definition: %s (%zu params, %zu state vars)\n", 
...
static void reprocess_property_bindings(KryonRuntime *runtime, KryonElement *element) {
    ...
    for (size_t i = 0; i < element->property_count; i++) {
        ...
        if (property && property->is_bound && property->binding_path) {
            ...
            fprintf(stderr, "   REF property %s binding=%s\n", property->name ? property->name : "<noname>", property->binding_path ? property->binding_path : "(null)");

                        fprintf(stderr, "🔄 DEBUG: Skipping REFERENCE property '%s' in reprocessing\n", 
    ...
    for (size_t i = 0; i < property->value.template_value.segment_count; i++) {
        ...
        if (segment->type == KRYON_TEMPLATE_SEGMENT_LITERAL) {
            ...
            if (var_value) {
                ...
                fprintf(stderr, "  🔗 Substituted variable '%s' = '%s'\n", var_name, var_value);
            } else {
                fprintf(stderr, "  ⚠️  Variable '%s' not found, using empty substitution\n", var_name ? var_name : "(null)");
            }
        }
    }
