...
func cEntrypoints(paths ...string) (methodSet, error) {
	...
	for _, path := range paths {
		...
		for _, match := range matches {
			...
			if endRel < 0 {
				return nil, fmt.Errorf("%s: unterminated declaration for %s", path, name)
			}
			out[name] = compactSignature(text[start : start+endRel+1])
		}
	}
	return out, nil
}
...
func checkForbidden(path string) {
	...
	check(err)
	text := string(data)
	for _, forbidden := range []string{`import "C"`, "Draw" + "UI", "UIText", "TextInputControl", "UIRender", "BeginDrawing", "EndDrawing", "github.com/waozixyz/kryon/go/kryui"} {
...
func render(native, cleanC methodSet) (string, bool) {
	...
	b.WriteString("| Method | Native Go (`go/kryon`) | Clean C entrypoint | Status |\n")
	b.WriteString("|---|---|---|---|\n")
