...
ifeq ($(KRYON_BACKEND),raylib)
  ...
  KRYON_BACKEND_SRCS = $(KRYON_NULL_BACKEND_C)
  KRYON_BACKEND_LIBS =
  KRYON_BACKEND_CFLAGS = -DKRYON_BACKEND_TERMI
  KRYON_BACKEND_LDLIBS =
...
KRYON_WEB_SYNC_CFLAGS = $(if $(filter 1,$(KRYON_WITH_SYNC)),-DHAS_LIBOQS=1 -DKRYON_HAS_LIBOQS=1 -I$(KRYON_WEB_LIBOQS_BUILD_DIR)/include,)
KRYON_WEB_SYNC_DEPS = $(if $(filter 1,$(KRYON_WITH_SYNC)),$(KRYON_WEB_LIBOQS_A),)
KRYON_WEB_SYNC_LIBS = $(if $(filter 1,$(KRYON_WITH_SYNC)),$(KRYON_WEB_LIBOQS_A),)
...
KRYON_ALL_SRCS := $(filter-out $(KRYON_DIR)/src/backend/termi_%.c,$(KRYON_ALL_SRCS))
endif
...
KRYON_NATIVE_LDLIBS += $(KRYON_LIBOQS_A)
endif
...
KRYON_NATIVE_CFLAGS += $(KRYON_NOTIFICATION_CPPFLAGS) $(KRYON_NOTIFICATION_CFLAGS)
KRYON_NATIVE_LDLIBS += $(KRYON_NOTIFICATION_LDLIBS)
KRYON_ACCESSIBILITY ?= 1
ifeq ($(shell uname -s),Linux)
ifeq ($(KRYON_BACKEND),raylib)
ifeq ($(KRYON_ACCESSIBILITY),1)
ifneq ($(shell pkg-config --exists gio-2.0 pango && printf yes),)
KRYON_NATIVE_CFLAGS += -DKRYON_ACCESSIBILITY_DBUS $(shell pkg-config --cflags gio-2.0 pango)
KRYON_NATIVE_LDLIBS += $(shell pkg-config --libs gio-2.0 pango)
endif
endif
endif
endif
...
CFLAGS = -Wall -Wextra -std=c99 -Os -fPIE $(KRYON_NATIVE_FEATURE_DEFS) -ffunction-sections -fdata-sections -DSUPPORT_FILEFORMAT_JPG=1 -DMINIZ_NO_ZLIB_COMPATIBLE_NAMES -DKRYON_EMBEDDED_ONLY=1 $(KRYON_SYNC_CFLAGS) $(KRYON_RUNTIME_ASSET_CFLAGS)
...
$(KRYON_ICON_ASSETS_C) $(KRYON_ICON_NAMES_C) $(KRYON_ICON_TYPES_H): $(KRYON_ICON_FILES) $(KRYON_DIR)/scripts/embed-icon-sheets.py | $(NATIVE_OBJ_DIR)
