...
| `src/ui/composition_input.zi`, `src/backend/composition_host.c` | Normal Ziran import and portable host queue for raw composition events; platform adapters still need to submit their events and keep retained composition bytes alive | checked Ziran plus platform host |
