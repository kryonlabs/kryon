...
/**
 * Recursive search for window by title
 */
static Window search_window_by_title(Display* display, Window root,
    ...
    if (title && strstr(title, title_pattern) != NULL) {
        // Verify window is visible
        if (is_window_visible(display, root)) {
            free(title);
            return root;
        }
    }
...
/**
 * Capture TaijiOS emulator window screenshot (high-level)
 */
bool capture_emulator_window(const ScreenshotOptions* options) {
    ...
    return capture_window_when_ready(window_title, options->output_path, timeout_ms);
}
