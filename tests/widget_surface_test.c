...
test_swatch_policy(void)
{
    ...
    check_int("color button checker a width", (int)paint.checker_a.width, 35);
...
test_list_box_layout_policy(void)
{
    ...
    StyleFrame list = {.value = {.fields = StyleContentOffset,
                                  .offset_y = 8.0f}};
    StyleFrame item = {.value = {.fields = StyleIconSize | StyleContentOffset,
                                  .icon_size = 30.0f, .offset_y = 30.0f}};
...
test_focusable_image_keyboard_navigation(void)
{
    ...
    Button((ButtonProps){.bounds=image.bounds,.id=621,.image=image});
    Button((ButtonProps){
...
main(int argc, char **argv)
{
    ...
    test_control_style_resolution();
    {
        ...
        const TreeNode *nodes = GetTreeNodes(&count);
        for(int i = 0; i < count; i++) {
            if(strcmp(GetNodeKindName(nodes[i].kind),"Input") == 0 &&
               nodes[i].id == 872) {
                paints++;
                check_int("numeric origin paint x", (int)nodes[i].bounds.x, 0);
                check_int("numeric origin retained id", nodes[i].id, 872);
            }
            if(nodes[i].id == 873) {
                next++;
...

    {
        ...
        int token;
        int previous_inspect = InspectEnabled();
        ...
        SetInspectEnabled(previous_inspect);
    }
