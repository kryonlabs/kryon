...
| Plots | `Plot` | covered |
| Menus | `Menu` (bar, popup, and context modes) | nested submenus plus focus, disabled/separator skipping, arrow/Home/End traversal, submenu entry/backout, activation, and Escape dismissal in native C and Go; lowered native entry points are internal host coverage, not public widget names |
...
| Drag and drop | `DragDrop` (typed source and target roles) | covered with typed copied payloads; lowered native source/target entry points are internal host coverage, not public widget names |
...
Native C and Go expose composed tab content through the same canonical
`TabBar`: callers keep selected state, store a non-negative returned index, and
submit arbitrary selected-tab children with ordinary conditionals. This keeps a
single tab renderer and avoids a separate tab scope API. Direct C and Go tests
plus generated C, Go, and JavaScript tests switch tabs by keyboard, activate
overlapping child buttons, and verify that only the selected tab's child can
receive input.
